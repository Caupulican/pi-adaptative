...
Options:
  ...
  --skip-binary-build  Do not rebuild the Bun binary (for reuse of a CI-built artifact)
  --help               Show this help
`);
}
...
function parseArgs() {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--skip-binary-build") {
			options.skipBinaryBuild = true;
			continue;
		}
		if (arg === "--out") {
			const value = args[++i];
