export function completeCiJobs() {
	return [
		{ name: "Build, check, test (ubuntu-latest)", steps: ["Verification-harness coverage gate", "Test non-coding-agent workspaces"] },
		{ name: "Build, check, test (windows-latest)", steps: ["Test non-coding-agent workspaces", "Test native incident collector with Windows PowerShell 5.1"] },
