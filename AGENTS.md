...
- Degenerate collapse drops later text that repeats an earlier block and later tool calls that share an execution identity (`getToolExecutionKey`, envelope fields omitted). The first tool-call id is kept so results still bind.
