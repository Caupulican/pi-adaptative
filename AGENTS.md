...
- CLOSED 2026-07-07: follow-up roadmap items R32-R38 closed the remaining Phase 4 tool-call reliability gaps: OpenAI-compatible streamed id synthesis (`2f39f10e`), truncated-args bounce (`3db63cf3`), schema-rich validation feedback (`c1aebcb2`), shape-only telemetry (`65945a12`), provider tool-name sanitization (`87026269`), replaying repaired args (`3d70c75a`), opt-in text tool-call extraction (`25cc5649`), repeated validation-failure escalation (`64df95f4`), the per-model adaptation store (`794868fd`), and in-band repair teach-back notes (pending commit). Keep the validation layer as validate-then-repair; do not reintroduce tool-specific pre-coercion shims.
