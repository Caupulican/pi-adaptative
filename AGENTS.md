...
4. **Run the release script**: the flow is split into `prepare` (bump/changelog/commit/push, no tag) and `promote` (gate on CI, then tag/push) so a CI failure never burns a version number.
   ...
   - Promote: finds the `Release vX.Y.Z` commit, polls GitHub Actions (`ci.yml`) for its conclusion on that exact commit SHA, and only on success creates and pushes the `vX.Y.Z` tag (which triggers `.github/workflows/build-binaries.yml`).
