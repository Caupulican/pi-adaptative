...
- Background workers notify by lane record plus a bounded completion event; untrusted output is retrieved explicitly via `delegate_status`. Late worker output never enters an active transcript — it races the foreground model.
...
- The worker claim envelope is authoritative, but non-JSON reports are retained as bounded plain text with disclosure, never rejected outright. Worker tool loops resolve tool-call protocol from the lane model, never the foreground model. Queue admission counts running workers only.
