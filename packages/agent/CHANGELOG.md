...
- `killTree` on Windows settles `already_dead` when taskkill finds no such process (the child exited between the liveness check and the kill), mirroring the POSIX path, instead of reporting a failed kill.
...
- Windows ancestry observation allows a bounded five seconds for concurrent process queries; refused termination now reports the specific observation or protected-target reason.
...
- A user message delivered while a run is in progress opens a new subject: verification failures the run produced before it stay active (listed, blocking goal completion) but no longer withhold the answer to the new subject. `prepareCompaction` accepts `activeTask`, a host-owned Active Task that replaces the latest user message, so an answered aside never becomes the checkpoint's task.
...
- Bounded receipt-derived reporting separates current-cycle observations from retained history, reconciles background handoffs and replay, and exposes missing or conflicting evidence without inventing execution outcomes.
