...
describe("completed tool cleanup settlement", () => {
	it.each(
		...
		const tool: AgentTool = {
			...
			async execute() {
				throw new Error("Must use bound executor");
			},
		...
		const messages = await runAgentLoop(
			...
			{ systemPrompt: "Fixture", messages: [], tools: [tool] },
			{
				model,
				toolConcurrency: 2,
				toolExecution: mode === "parallel" ? "parallel" : "sequential",
				convertToLlm: (history) =>
					history.filter((m) => m.role === "user" || m.role === "assistant" || m.role === "toolResult"),
			...
			() => {
				...
				stream.push({
					...
					message: {
						...
						content: first
							? (mode === "parallel" ? ["first", "sibling"] : ["first"]).map((id) => ({
									type: "toolCall",
									id,
									name: tool.name,
									arguments: {},
		...
		expect(cleanups.filter((entry) => entry.endsWith(":first"))).toEqual([
			"registered:first",
			"second:first",
			"start:first",
			"binding:first",
		...
		if (operation === "success") {
			...
			expect(Object.getOwnPropertyDescriptor(result?.details, "piVerification")).toMatchObject({
				value: { id: "check-first", status: "passed" },
				configurable: false,
				writable: false,
			});
		}
		if (mode === "parallel") {
			...
			expect(retainedToolInvocation(results[1].details)).toMatchObject({
	...
	it.each(["completed", "manual"].flatMap((race) => ["throw", "control"].map((fault) => ({ race, fault }))))(
		"retains completion across unsubscribe: $race / $fault",
		async ({ race, fault }) => {
			const release = Promise.withResolvers<void>();
			let effects = 0;
			let handoffs = 0;
				...
				() => {
					...
					stream.push({
						...
						message: {
							...
							timestamp: 0,
							stopReason: "toolUse",
							content: [{ type: "toolCall", id: "first", name: tool.name, arguments: {} }],
						},
					});
					return stream;
				},
			...
			expect(retainedToolInvocation(result?.details)).toMatchObject({
				operationStatus: "success",
				postprocessingFailures: fault === "throw" ? ["cleanup"] : [],
			});
