...
describe("preparation cleanup registration", () => {
	it.each(["execute", "background", "block", "cancel", "hook_throw", "reservation_throw", "selector_throw", "cleanup_throw"] as const)(
		"releases registered resources and the binding after %s", async (boundary) => {
			...
			const tool: AgentTool<typeof parameters> = {
				name: "fixture", label: "Fixture", description: "Preparation lifetime", parameters,
				async execute() { throw new Error("Expected bound tool"); },
			...
			await runAgentLoop(
				...
				{ systemPrompt: "", messages: [], tools: [tool] },
				{
					...
					convertToLlm: (history) => history.filter((message): message is Message =>
						["user", "assistant", "toolResult"].includes(message.role)),
					toolExecution: "sequential", maxProviderTurns: 1,
					...
					onToolCallStart: () => {
						...
						return { release() { order.push("start"); } };
					},
				...
				() => {
					const message: AssistantMessage = {
						role: "assistant", content: [{ type: "toolCall", id: "fixture", name: tool.name, arguments: {} }],
						api: "openai-responses", provider: "openai", model: "fixture", stopReason: "toolUse", timestamp: 1, usage: createEmptyUsage(),
					};
			...
			expect(order).toEqual([
				...(boundary === "execute" || boundary === "background" ? ["execute"] : []), "first", "second",
				...(boundary === "execute" || boundary === "background" || boundary === "selector_throw" ? ["start"] : []), "binding",
			]);
