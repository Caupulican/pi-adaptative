...
describe("pre-execution cancellation recovery", () => {
	it.each(["cancel", "preflight_error", "blocked"] as const)("preserves %s semantics before execution", async (outcome) => {
		...
		const tool: AgentTool<typeof parameters> = {
			name: "preflight_fixture", label: "Preflight fixture", description: "Preflight recovery fixture", parameters,
			async execute() { throw new Error("Expected the bound executor"); },
			async bindInvocation() {
				return {
					...
					async execute() { executions++; return { content: [{ type: "text", text: "executed" }], details: {} }; },
					release() { releases++; },
				};
			},
		};
		const messages = await runAgentLoop(
			...
			() => {
				const message: AssistantMessage = {
					role: "assistant", content: [{ type: "toolCall", id: "fixture-call", name: tool.name, arguments: { command: "fixture" } }],
					api: "openai-responses", provider: "openai", model: "fixture", usage: createEmptyUsage(), stopReason: "toolUse", timestamp: 1,
				};
