...
const tool: AgentTool<typeof parameters> = {
	name: "receipt_fixture", label: "Receipt fixture", description: "Recovery replay fixture", parameters,
	failureRecovery: { getTimeoutMs: ({ timeout }) => (timeout ?? 60) * 1000 },
	async execute() { throw new Error("Replay must not execute"); },
};

function append(messages: AgentMessage[], command: string, outcome: Partial<ToolResultMessage>): void {
	const id = `call-${messages.length}`;
	messages.push({
		role: "assistant", content: [{ type: "toolCall", id, name: tool.name, arguments: { command, timeout: 60 } }],
		api: "openai-responses", provider: "openai", model: "fixture", usage: createEmptyUsage(),
		stopReason: "toolUse", timestamp: 1,
	}, {
		role: "toolResult", toolCallId: id, toolName: tool.name, content: [], isError: true, timestamp: 1, ...outcome,
	});
}
...
describe("executor receipt restoration", () => {
	...
	it.each(["timeout", "exit_1", `${"x".repeat(47)}…`])("preserves %s despite misleading raw output", (failureCode) => {
		...
		append(messages, "fixture", {
			...
			details: receiptDetails({
				version: 1, requestId: "fixture", execution: "completed", operationStatus: "error",
				failureCode, postprocessingFailures: [],
			}),
		});
		const result = JSON.parse(JSON.stringify(messages[1])) as ToolResultMessage;
		expect(restoreToolFailureRecord(result, tool.name, { command: "fixture", timeout: 60 }))
			.toMatchObject({ failureCode, phase: "execution" });
		const gate = new ToolFailureRecoveryGate();
		gate.restoreFromMessages(messages, [tool]);
		expect(gate.admit(tool, { command: "fixture", timeout: 120 }, undefined))
			.toMatchObject({ kind: failureCode === "timeout" ? "allowed" : "blocked" });
	});
	...
	it.each(["fixture", "other"])("does not treat a running %s call as corrective success", (command) => {
		...
		append(messages, command, {
			isError: false, content: [{ type: "text", text: "Running in background" }],
			details: receiptDetails({ version: 1, requestId: "background", execution: "running", postprocessingFailures: [] }),
		});
		const persisted = JSON.parse(JSON.stringify(messages)) as AgentMessage[];
		const gate = new ToolFailureRecoveryGate();
		gate.restoreFromMessages(persisted, [tool]);
		...
		persisted.push({
			...
			isError: false,
			details: receiptDetails({
				version: 1, requestId: "finished", execution: "completed", operationStatus: "success", postprocessingFailures: [],
			}),
		});
		const afterSuccess = new ToolFailureRecoveryGate();
		afterSuccess.restoreFromMessages(persisted, [tool]);
		expect(afterSuccess.admit(tool, { command: "fixture", timeout: 120 }, undefined)).toMatchObject({ kind: "allowed" });
	});
	...
	it("does not count interruption as a failure or corrective success", () => {
		...
		append(messages, "interrupted", {
			...
			details: receiptDetails({
				version: 1, requestId: "interruption", execution: "unknown", failureCode: "aborted", postprocessingFailures: [],
			}),
		});
		const persisted = JSON.parse(JSON.stringify(messages)) as AgentMessage[];
		const gate = new ToolFailureRecoveryGate();
