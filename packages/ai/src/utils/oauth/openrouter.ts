...
type OpenRouterCallbackServer = {
	callbackUrl: string;
	credential: Promise<OAuthCredentials>;
	close(): void;
};
...
async function exchangeAuthorizationCode(
	...
	return { access: body.key, refresh: "", expires: Number.MAX_SAFE_INTEGER };
}
...
async function startCallbackServer(
	...
	let resolveCredential: (credential: OAuthCredentials) => void = () => {};
	let rejectCredential: (error: Error) => void = () => {};
	const credential = new Promise<OAuthCredentials>((resolve, reject) => {
	...
	const finish = (result: { credential: OAuthCredentials } | { error: Error }): void => {
		if (settled) return;
		settled = true;
...
export async function loginOpenRouter(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
	...
	const callback = await startCallbackServer(callbackPath, verifier, callbacks.signal);
	...
	callbacks.onProgress?.(`Listening for OpenRouter OAuth callback on ${callback.callbackUrl}`);
	callbacks.onAuth({ url: authorizeUrl.toString(), instructions: "Complete sign-in in your browser." });
	try {
