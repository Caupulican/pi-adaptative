...
- The built-in xAI catalog test lists Grok 4.7 and Grok 4.7 Fast with Grok 4.5 and Grok 4.6.
...
- Updated Grok CLI OAuth requests to match the installed v1.0.13 subscription client identity.
...
- GPT-6 Astra on the OpenAI and Codex providers (`gpt-6-astra`). From the OpenAI model page of 2026-09-03: $10 input, $1 cached input, $12.5 cache write and $50 output per million tokens, 2x input and cache and 1.5x output above 272k input tokens (carried as a `cost.tiers` entry, the advertised window stopping at the threshold as for GPT-5.6), 128,000 max output, prompt caching, text and image input, reasoning `low` through `max` on the API with `medium` as the default. The Codex entry, from Codex's bundled catalogue of the same day, adds the Responses Lite contract, `low` as the default and `ultra` as Astra's own wire effort ("maximum reasoning with automatic task delegation") on top of `max`; it mirrors the list prices since ChatGPT plans are not billed per token. The GPT-5.6 cache contract (explicit cache mode, caching off when retention is none) applies to GPT-6. The table entry is a fallback: models.dev takes over the moment it lists the model. Codex still hides Astra from its own picker while it rolls out, and the `openai-codex` default stays GPT-5.6 Sol.
