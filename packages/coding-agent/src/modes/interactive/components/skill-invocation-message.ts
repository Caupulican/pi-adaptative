import { Box, Markdown, type MarkdownTheme, Text } from "@caupulican/pi-tui";
...
export class SkillInvocationMessageComponent extends Box {
	...
	private updateDisplay(): void {
		...
		if (this.expanded) {
			...
			this.addChild(new Text(renderTitleBadge(theme, { label: "skill" }), 0, 0));
			...
			this.addChild(
				new Text(
					renderTitleBadge(theme, {
						label: "skill",
						details: [
							{ text: this.skillBlock.name, color: "customMessageText" },
