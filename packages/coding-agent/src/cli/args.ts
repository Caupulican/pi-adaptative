	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--") {
			for (let j = i + 1; j < args.length; j++) {
				const restArg = args[j];
				if (restArg.startsWith("@")) {
					result.fileArgs.push(restArg.slice(1));
				} else {
					result.messages.push(restArg);
				}
			}
			break;
...
${chalk.bold("Environment Variables:")}
  ...
  PI_SHARE_VIEWER_URL              - Base URL for /share command (default: GitHub Gist URL)
