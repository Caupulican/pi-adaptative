...
import type { IntegrationLockAcquisition, IntegrationLockOwner, LaneRegistration, WorktreeSyncEpoch } from "./codes.ts";
...
export async function acquireIntegrationLock(
	paths: SyncStorePaths,
	owner: Omit<IntegrationLockOwner, "acquiredAt">,
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		if (await tryMkdirLock(paths)) {
			const manifest: IntegrationLockOwner = { ...owner, acquiredAt: now() };
			await fsPromises.writeFile(paths.lockOwnerFile, `${JSON.stringify(manifest)}\n`, "utf-8");
			await appendAuditEvent(paths, { event: "lock_acquired", ...(attempt > 0 ? { takeover: true } : {}) }, now());
			return { acquired: true, takeover: attempt > 0 };
		}
		...
		if (holder) {
			...
			try {
				...
				// The dir vanished between mkdir-fail and stat (concurrent release): just retry.
				continue;
			}
			if (nowMs() - dirMtimeMs < OWNERLESS_LOCK_STALE_MS) {
				return { acquired: false, holderAlive: true };
			}
		}

		// Dead owner (or stale ownerless dir): take over, audited, then retry the atomic mkdir once.
		// A concurrent taker racing us is fine -- exactly one mkdir wins on the next attempt.
		await fsPromises.rm(paths.lockDir, { recursive: true, force: true });
		await appendAuditEvent(
			paths,
			{ event: "lock_takeover", ...(holder ? { deadOwnerPid: holder.pid, deadOwnerLaneKey: holder.laneKey } : {}) },
			now(),
		);
	}
	const holder = await readLockOwner(paths);
	return { acquired: false, holder, holderAlive: holder ? (deps.isPidAlive ?? defaultIsPidAlive)(holder) : true };
}

export async function releaseIntegrationLock(paths: SyncStorePaths, deps: IntegrationLockDeps = {}): Promise<void> {
	const now = deps.now ?? (() => new Date().toISOString());
	await fsPromises.rm(paths.lockDir, { recursive: true, force: true });
