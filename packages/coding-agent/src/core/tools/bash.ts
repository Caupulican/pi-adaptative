...
import { classifyCommandFamily } from "./command-family.ts";
import {
	...
	return {
		...
		async execute(
			...
			const createExitError = (text: string, exitCode: number, effectiveCwd: string) => {
				const coverage =
					/No files were processed in the specified paths/i.test(text) &&
					classifyCommandFamily(command).family === "diagnostics"
						? "\nNo files were processed. Inspect path existence and the checker's include/ignore rules."
						: "";
				return new AgentToolExecutionError(
					appendStatus(`${text}${coverage}`, `Command exited with code ${exitCode}\ncwd: ${effectiveCwd}`),
					`exit_${exitCode}`,
