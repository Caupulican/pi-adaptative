	...
	it.each(["selector_throw", "host_cancel", "execute", "rewritten_id"] as const)(
		...
		async (boundary) => {
			...
			const shell: AgentTool<typeof parameters> = {
				name: "shell_fixture", label: "Shell", description: "Fixture command", parameters,
				async execute(id) {
					return scope.runExclusive(async () => {
						executed.push(id);
				...
				const messages = await runAgentLoop(
					...
					() => {
						const message: AssistantMessage = {
							...
							api: "openai-responses", provider: "openai", model: "fixture", stopReason: "toolUse", timestamp: 1,
