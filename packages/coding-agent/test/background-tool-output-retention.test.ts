...
function controlledCall(negative = false) {
	...
	const finished: BackgroundToolCallCompletion = {
		...
		result: {
			content: [
				{ type: "text", text: Array.from({ length: 5000 }, (_, index) => `result line ${index}`).join("\n") },
			],
			usage,
			details: {
				piToolInvocation: {
					...
					requestId,
					execution: "completed",
					operationStatus: negative ? "error" : "success",
					postprocessingFailures: ["cleanup"],
				},
				piVerification: {
					version: 1,
					id: "retention-verification",
...
describe("background output retention failures", () => {
	...
	it.each(
		["lookup", "write", "reference", "control"].flatMap((fault) =>
			["success", "negative", "canceled"].map((operation) => ({ fault, operation })),
		),
	)(
		"publishes the operation terminal and partial-output disclosure: $fault / $operation",
		async ({ fault, operation }) => {
			vi.useFakeTimers();
			...
			const lookup = vi.fn(() => {
				if (fault === "lookup") throw new Error("private lookup failure");
				return store;
			});
			const controller = new BackgroundToolTaskController({
				getSessionId: () => "fixture-session",
				getArtifactStore: lookup,
				persist: (record) => {
					persisted.push(JSON.parse(JSON.stringify(record)));
				},
			...
			let settled = false;
			const waiting = controller.wait("tool-task-1").then((record) => {
				settled = true;
				return record;
			});
			...
			expect(result.status).toBe(
				operation === "success" ? "completed" : operation === "negative" ? "failed" : "canceled",
			);
			expect(result.usage).toEqual(usage);
			expect(result.piToolInvocation).toMatchObject({
				operationStatus: operation === "negative" ? "error" : "success",
				postprocessingFailures: ["cleanup"],
			});
			...
			const restored = new BackgroundToolTaskController({
				...
				loadPersistedRecordsNewestFirst: () => [...persisted].reverse(),
				persist: () => {},
				notifyTerminal: () => {},
			});
			expect(restored.list()[0]).toMatchObject({
				...
				piToolInvocation: result.piToolInvocation,
			});
	...
	it("still refuses successful delivery when the terminal record cannot be persisted", async () => {
		...
		expect(notifications).toEqual([
			expect.objectContaining({ status: "failed", output: expect.stringContaining("could not be persisted") }),
		]);
