...
describe("script result survives optional artifact failure", () => {
	it.each(["write", "reference", "control"].flatMap((fault) =>
		...
		const settled = await tool.execute("fixture-call", { script: "fixture" }, undefined, undefined, {} as ExtensionContext).then(
		...
		if (operation === "timeout") {
			...
			expect(settled.result.details).toMatchObject({ outcome: operation === "success" ? "executed" : "failed", exitCode, durationMs: 10 });
			text = settled.result.content.filter((block) => block.type === "text").map((block) => block.text).join("\n");
		}
