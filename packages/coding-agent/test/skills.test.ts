...
describe("skills", () => {
	describe("loadSkillsFromDir", () => {
		...
		it("reads only bounded frontmatter during discovery, regardless of body size", () => {
			...
			try {
				...
				const bodyMarker = "BODY_MUST_NOT_BE_READ_DURING_DISCOVERY";
				writeFileSync(
					filePath,
					`---\nname: large-skill\ndescription: Metadata-only discovery.\n---\n\n${"x".repeat(8 * 1024 * 1024)}${bodyMarker}`,
				);

				const frontmatter = readSkillFrontmatterFile(filePath);
				const result = loadSkillsFromDir({ dir: root, source: "test" });

				expect(Buffer.byteLength(frontmatter, "utf8")).toBeLessThan(1024);
				expect(frontmatter).not.toContain(bodyMarker);
				expect(result.skills.map((skill) => skill.name)).toEqual(["large-skill"]);
	...
	describe("loadSkills with options", () => {
		const emptyAgentDir = resolve(import.meta.dirname, "fixtures/empty-agent");
		const emptyCwd = resolve(import.meta.dirname, "fixtures/empty-cwd");
