...
describe("background terminal observer failures", () => {
	...
	it.each(
		["usage", "live", "both", "control"].flatMap((fault) =>
		...
		vi.useFakeTimers();
		const onError = vi.fn(() => {
			if (reporter === "throws_undefined") throw undefined;
		});
	...
	it("admits completion ownership when the live subscriber and its diagnostic both throw", async () => {
		...
		expect(notifications).toEqual([
			expect.objectContaining({ status: "completed", output: "committed:fixture-call" }),
		]);
		await controller.shutdown();
	});
	...
	it.each(["initial", "terminal"])("retains the %s persistence refusal when diagnostics throw", async (boundary) => {
		...
		const { controller, notifications } = createHarness({
			persist: () => {
				if (++writes > (boundary === "initial" ? 0 : 1)) throw new Error("write failed");
			},
			onError: () => {
				throw undefined;
			},
	...
	it.each(["lookup", "remove", "cleanup", "control"])(
		"notifies a new task despite old artifact pruning: %s",
		async (fault) => {
			...
			await controller.waitForNotifications();
			expect(notifications).toEqual([
				expect.objectContaining({ taskId: "tool-task-65", status: "completed", output: "committed:fixture-call" }),
			]);
