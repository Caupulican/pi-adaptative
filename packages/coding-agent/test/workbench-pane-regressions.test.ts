...
describe("Workbench review regressions", () => {
	...
	it("prioritizes a folded failure receipt over the work heading in a one-row combined pane", () => {
		...
		controller.record(new Text("failed output", 0, 0), workbenchToolObservation("failed", true));
		controller.complete();
		expect(
			stripAnsi(
				view
		...
		).toContain("retained: 1 error results");
		controller.dispose();
	});
	...
	it("keeps the same evidence scroll position across unrelated receipts, resets for new evidence", () => {
		...
		controller.record(
			new Text(Array.from({ length: 20 }, (_, i) => `diff${i}`).join("\n"), 0, 0),
			workbenchToolObservation("diff"),
		);
		view.render(100);
		...
		controller.record(undefined, workbenchToolObservation("unrelated"));
		const after = view
			.render(100)
			.map(stripAnsi)
			.slice(view.upperTop + 1, view.upperTop + view.upperHeight);
		expect(after).toEqual(before);
		controller.record(new Text("new evidence", 0, 0), workbenchToolObservation("new"));
	...
	it("shows every preview of the cycle in order and follows the newest until the operator scrolls up", () => {
		...
		for (const name of ["first", "second", "third"])
			controller.record(new Text(`${name} result`, 0, 0), workbenchToolObservation(name));
		let text = stripAnsi(
			view
				.render(100)
				.slice(view.upperTop, view.upperTop + view.upperHeight)
		...
		controller.record(new Text("newest", 0, 0), workbenchToolObservation("newest"));
		text = stripAnsi(
			view
				.render(100)
				.slice(view.upperTop, view.upperTop + view.upperHeight)
