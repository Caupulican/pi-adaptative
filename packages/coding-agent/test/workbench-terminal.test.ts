import { type Component, Container, CURSOR_MARKER, Text, TUI } from "@caupulican/pi-tui";
...
describe("Workbench terminal rendering", () => {
	...
	it("keeps pane boundaries and hardware cursor through completion, resize and overlay focus restoration", async () => {
		...
		try {
			...
			expect(terminal.getViewport().at(-1)).toContain("> prompt");
			expect(terminal.getCursorPosition().y).toBe(29);
			view.setInspector(["Work complete"]);
			view.setExecution(undefined);
			ui.requestRender();
			await terminal.waitForRender();
			expect(terminal.getViewport()[0]).toContain("Work complete");
			expect(terminal.getViewport()[1]).toContain("Conversation");
			const overlay = ui.showOverlay(new Text("question dialog", 0, 0));
			await terminal.waitForRender();
			overlay.hide();
			terminal.sendInput("x");
			await terminal.waitForRender();
			expect(terminal.getViewport().at(-1)).toContain("> promptx");
			for (const [columns, rows] of [
				...
				await terminal.waitForRender();
				expect(terminal.getViewport().at(-1)).toContain("> promptx");
				expect(terminal.getCursorPosition().y).toBe(rows! - 1);
			}
		} finally {
			ui.stop();
		}
