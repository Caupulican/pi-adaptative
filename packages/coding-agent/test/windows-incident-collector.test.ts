...
const NATIVE_POWERSHELL_PROCESS_TIMEOUT_MS = 60_000;
const NATIVE_COLLECTOR_TEST_TIMEOUT_MS = NATIVE_POWERSHELL_PROCESS_TIMEOUT_MS * 2 + 15_000;
...
describe.runIf(process.platform === "win32" || powerShell !== undefined)("native Windows incident collector", () => {
	it(
		"packages the affected evidence and excludes credential/configuration stores",
		() => {
			...
			const collected = spawnSync(
				...
				],
				{
					encoding: "utf8",
					env: collectorEnv,
					timeout: NATIVE_POWERSHELL_PROCESS_TIMEOUT_MS,
					windowsHide: true,
				},
			);
			...
			const extracted = spawnSync(
				...
				{ encoding: "utf8", timeout: NATIVE_POWERSHELL_PROCESS_TIMEOUT_MS, windowsHide: true },
			);
			...
			expect(readFileSync(join(extractedDir, "diagnostics", "environment.txt"), "utf8")).toContain(
				"selected_session=",
			);
		...
		NATIVE_COLLECTOR_TEST_TIMEOUT_MS,
	);
});
