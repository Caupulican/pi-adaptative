...
describe("tool-owned failure recovery contracts", () => {
	...
	it.each([
		{ cwd: "/fixture workspace", pathFlavor: "posix" as const },
		{ cwd: "C:\\fixture workspace", pathFlavor: "win32" as const },
		{ cwd: "\\\\fixture-host\\share\\workspace", pathFlavor: "win32" as const },
	])(
		"carries one bounded verification id from failure to green in $pathFlavor ($cwd)",
		async ({ cwd, pathFlavor }) => {
			const outputDirectory = await createTemporaryRoot("pi-shell-verification-metadata-");
				...
				operations: {
					exec: async (shellCommand, _cwd, options) => {
						if (shellCommand === command) {
							verificationRuns++;
							options.onData(
								Buffer.from(verificationRuns === 1 ? "Tests 1 failed (1)\n" : "Tests 1 passed (1)\n"),
							);
