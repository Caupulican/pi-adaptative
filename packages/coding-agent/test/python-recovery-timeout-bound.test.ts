...
function fixture(result: PythonExecutionResult, capture?: (request: PythonExecutionRequest) => void) {
	return createPythonTool("/fixture", {
		...
		resolveRuntime: async () => ({ status: "ready", uvPath: "/fixture/uv", pythonPath: "/fixture/python", pythonInstalled: false }),
		...
		operations: {
			...
			exec: async (request) => { capture?.(request); return result; },
		},
	});
}
...
describe("Python timeout recovery authority", () => {
	it.each([
		[undefined, 30_000], [0, 30_000], [-1, 30_000], [Number.NaN, 30_000], [Infinity, 30_000],
		[0.1, 1000], [1.9, 1000], [60, 60_000], [600, 300_000],
	])("uses the executor bound for timeoutSeconds=%s", async (timeoutSeconds, milliseconds) => {
		let captured: number | undefined;
		const tool = fixture({ reason: "exited", exitCode: 0, signal: null }, (request) => { captured = request.timeoutMs; });
	...
	it.each([
		...
		await expect(tool.execute("terminal", { code: "pass" })).rejects.toMatchObject({
			failureCode, errorKind, outputSignature: expect.any(String),
		});
	});
	...
	it.each([
		...
		try { await tool.execute("timeout", args); } catch (error) {
		...
		gate.apply({ kind: "unproductive", tool, args, record: describeOperationOutcome(tool.name, args, failure.failureCode, failure.message) });
