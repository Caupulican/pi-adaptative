...
describe("FitnessRoleSelectorComponent", () => {
	...
	it("pre-selects a suggestion's shaped role so one keypress confirms it", () => {
		// The whole point of the suggestion flow: a model shaped as the expensive tier lands on
		// "router-expensive" already highlighted, not on the index-0 default — Enter confirms without navigation.
		const onSelect = vi.fn();
		const selector = new FitnessRoleSelectorComponent("ollama/some-model", onSelect, () => {}, "router-expensive");
		selector.handleInput("\n");
		expect(onSelect).toHaveBeenCalledWith("router-expensive");
	});
