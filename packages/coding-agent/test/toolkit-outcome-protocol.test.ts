...
function fixture(execute: () => Promise<ScriptExecution>) {
	return wrapToolDefinition(
		createRunToolkitScriptToolDefinition({
			getScripts: () => [{ name: "fixture", description: "Synthetic fixture", runner: "bash", path: "fixture.sh" }],
			execute,
		}),
	);
}
...
describe("toolkit execution classification in the real core loop", () => {
	it.each([
		...
		{ name: "deadline_zero", exitCode: 0, timedOut: true, completed: true, native: true, failureCode: "timeout" },
		{
			...
			completed: false,
			native: false,
			failureCode: "toolkit_execution_incomplete",
		},
		{
			name: "nan_exit",
			exitCode: Number.NaN,
		...
		"keeps $name distinct from other terminal kinds",
		async ({ name, exitCode, timedOut, completed, native, failureCode }) => {
				...
				() => {
					...
					stream.push({
						...
						message: {
							...
							stopReason: "toolUse",
							content: [
								{ type: "toolCall", id: "fixture-call", name: tool.name, arguments: { script: "fixture" } },
							],
						},
					});
					return stream;
