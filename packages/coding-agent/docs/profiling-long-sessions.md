...
- **Host long-session profile** — `packages/coding-agent/test/profiling/host-long-session.profile.test.ts`.
  Drives a REAL `AgentSession` through the faux provider with the tool mix that dominates real
  sessions (task_steps, get_goal, update_goal, read) for N tool turns, records every tool call and
  every request's host assembly time from the session-log markers above, prints them by session
  decile, and writes a V8 `.cpuprofile`. Skipped unless opted in.
