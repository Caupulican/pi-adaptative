...
When running inside a Herdr environment (`HERDR_ENV=1` with valid pane, workspace, tab and socket), `placement` defaults to `current-pane`. This attaches to the active shared Herdr daemon and splits sibling panels directly in the caller's active tab using `--no-focus`, keeping the user's current focus intact without creating an isolated server or workspace. Specifying `placement: "managed-workspace"` creates a private, isolated daemon. Partial launch failures in `current-pane` mode close only newly created panes verified to match the intended occupant; the caller pane and tab are never touched. The endpoint socket and caller executable (`HERDR_BIN_PATH`, which is strictly required for `current-pane` placement without unverified fallbacks) are validated and saved immutably into `job.socketPath` and `job.binPath` at admission, ensuring that helper processes and restart recoveries use the exact admitted binding and protocol (supporting both pinned managed daemon protocol 20 and live daemon protocol 22) without consulting ambient environment variables.

Provide each `agents` entry with a distinct `task` responsibility when launching a team against one objective. If the top-level task is omitted in `fire_task`, distinct per-agent tasks are synthesized into the team objective. The default team separates implementation, validation and review. `command` accepts a structured native executable/wrapper plus arguments and leading environment assignments. Authentication probes that same executable, environment and working directory. Shell pipelines are not inferred to be authenticated native providers. `model` selects a native model; `apiProvider` is Pi-only. For `agy`, the selected `model` is validated against the local `agy models` catalog. External CLIs configure model reasoning effort through native arguments (e.g. `agy` with `model: "gemini-3.8-flash-high"` and `args: ["--effort", "high"]`); `thinkingLevel` is Pi-only. Conflicting selectors and unprobed authentication overrides fail admission. Do not put credentials in arguments or environment specifications because job configuration persists.
...
The controller separately awaits a stopped-work event, then rechecks the same pane occupant, state sequence and agent revision before accepting the exact-turn claim. An early `idle` event alone is treated as a transient state: completion requires both an authenticated final report and a matching settled native occupant (including report-before-stop flows where the occupant settles out of `working`). Herdr's snapshot revision is not an agent-state version and cannot establish this fence. Printed markers, prompt echoes and TUI footers never prove success. Truncated terminal display does not invalidate a separately authenticated claim. Missing claims require review; native blocked/question state takes precedence over a claimed success.

A stopped worker's question reaches the owning parent immediately even if another teammate is still working. `answer_question` targets its `jobId` and `agentId`, with `answer.text` or `answer.keys`. It resumes the original task in the same native context, with a fresh dispatch identity; the worker must query `current` again before reporting. `send_followup` starts another task in that same persistent session. When steering a running `agy` agent, `send_followup` admits exclusive steering in the job ledger, sends `esc` to interrupt the active generation, waits event-driven for the matching native occupant to settle into an `idle`/`done` ready boundary, finishes the interrupted turn once, submits the successor prompt, and verifies acceptance. When an agent is actively working, passing `steer: false` explicitly rejects before external input rather than interrupting or silently queuing.
...
`stop_job` and `stop_session` terminate exact owned agent panes directly without requiring magic confirmation tokens, closing the job's admitted agent panes while preserving shared daemons, caller panes, and unrelated sessions. Pass `dryRun: true` to preview termination targets without closing panes. On shared `current-pane` sessions, stopping a job or session closes the team's agent panels while preserving the shared daemon, caller pane, and sibling workspaces. `dismiss` ends tracking without killing an idle native session; active work must be stopped first. `archive_job` moves eligible records into a bounded recoverable archive. Reusing a `launchKey` with `force:true` requires the previous native agents to be closed.
