...
Rows, top to bottom: a title strip (application name, the plan or goal naming the work or else the session name, and the run state WORKING / WAITING / IDLE on the right), the live activity line, the work area, its divider, the conversation header, conversation rows, the status band (transient notices, extension widgets and session status on a surface tone), the input, extension widgets below it, and one key-hint row. Zones separate by surface tone (`workbenchSurface`, optional in custom themes and defaulting to the custom-message background) and spacing; nothing is framed.
