...
For TypeScript definitions in your project, inspect `node_modules/@caupulican/pi-adaptative/dist/`.
...
Above the early trigger (`compaction.triggerPercent`), compaction runs only when it is priced to pay. The price compares resuming on the cached prefix with compacting now: the summarizer's read of the prefix, the summary at the output price, writing the compacted prefix, and the shorter prefix read by every request the history is expected to make. Each input is learned from the decision ledger: the cache share the provider keeps after the lane's real idle gap (its survival curve), how many requests a history lasts between compactions, and what past compactions left and generated. Without a recorded compaction there is nothing to price with, and only the hard boundary compacts. Each verdict is recorded as an `early_compaction` cache decision. A summarizer on the session's own model sends the context exactly as the session last sent it plus one instruction, so the provider serves the prefix from its cache.

While the session's model idles (its reply finished and it waits for you or for a tool), Pi may prepare a compaction ahead of time. From the same learned facts it picks the moment when a summary still reads the prefix from cache and is most likely to be needed: your usual return time, or the tool's, past the point where the provider's cache is expected to be gone. It summarizes then, on the session lane, and records the result as a `compaction_prepared` entry without applying it. Nothing is planned when the lane keeps its cache or when that value is not positive. At the next request (your message, or a long tool's results) the admission gate compares continuing from the prepared summary with resuming on the full history at the real idle gap, and applies the summary only when that is cheaper. The assistant message that issued pending tool calls and all their results stay in the kept history. A message you send while a summary is being prepared cancels the preparation. A prepared summary is never used after a request has gone out on the full history.
...
```typescript
import { convertToLlm, serializeConversation } from "@caupulican/pi-adaptative";
