...
- **Bounded Goal Continuation & Transient Recovery (`goal-session-controller.ts`, `goal-state.ts`):**
  ...
  - *Distinct Runaway Signatures:* Consumed runaway signatures live in one bounded durable collection (`consumedRunawaySignatures`, 16 entries) separate from provider failure accounting: one automatic resume per signature, alternating signatures cannot re-earn one, a full collection refuses rather than evicts, runaway stops do not increment the provider failure streak, and neither provider successes nor trusted evidence reopen a consumed signature. Only the owner's own prompt restarts the allowance.
...
> Correction (2026-09-12, independent review): the verification claims below were recorded before two GitHub CI candidates (runs 34720858126 and 34721873259) failed 9 of 10 jobs each. The targeted runs listed here never included the suites that pin the changed behaviour (`agent-session-runaway-escalation`, `system-prompt-builder-tool-selection`, `python-tool-registration`, `learning-defaults-on`, the patch's own `autonomy-handoff`), and `npm run check` runs only script, installer and guard tests (never the application test suites), so neither is evidence of a green suite. The `--bail=1` sharded CI shows at most one failure per shard; the full failure set was established by focused local runs afterwards (see `docs/doctrine.md` rows dated 2026-09-12 for the resulting contract changes). Treat the counts below as historical, not as acceptance evidence.
...
- **Honest Limitations & System Boundaries:**
  ...
  - The direct-shell script gate recognizes explicit invocation forms parsed with the POSIX grammar of the `bash` tool on every platform: registered forward-slash spellings and quoted Windows paths are gated; an unquoted backslash spelling (`.\scripts\calc.ps1`) is the escape sequence for a different name and is neither gated nor an execution of the registered script. `source`, `.`, pipe-to-shell and redirection forms remain a documented best-effort limit, not a sandbox claim.
  - The keyword `risk_assessment` gate was removed deliberately; Python source and shell text are not read for intent. Structural envelope denials (missing `process.exec`, denied/unlisted tools, path scope) are the remaining automatic rejections.
