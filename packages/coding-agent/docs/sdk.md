...
The `Agent` class (from `@caupulican/pi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@caupulican/pi-adaptative";
...
import { createAgentSession, defineTool } from "@caupulican/pi-adaptative";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@caupulican/pi-adaptative";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@caupulican/pi-adaptative";
