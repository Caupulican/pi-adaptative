...
Configure delivery in [settings](docs/settings.md): `steeringMode` (default `"all"`: every queued steer reaches the next model turn together) and `followUpMode` (default `"one-at-a-time"`, waits for a response) can each be `"one-at-a-time"` or `"all"`. `transport` selects provider transport preference (`"sse"`, `"websocket"`, or `"auto"`) for providers that support multiple transports.
...
Working preferences are learned from the owner's own words: each owner prompt is recorded as evidence, a `memory` write to `USER.md` cites it (scope, explicit or inferred basis, quoted sources), the host verifies the citations and counts independent observations, an explicit supported preference applies at once, an inferred pattern needs two independent owner observations and never overrides an explicit line, and everything else stays an audited candidate. Handoffs to in-process workers and Herdr teams receive the applicable preferences upfront. Working preferences stored in `USER.md` reach the model in the same session they change: the static memory block stays frozen for prompt-cache stability, and a committed `USER.md` mutation is delivered to the next provider request as one `user_persona` record (bounded to the memory prompt budget in whole lines, recorded once per revision, carried across compaction, explicitly cleared when the file and the installed block agree again or memory is disabled). Every projection of `USER.md` (root prompt, worker `memory_read` snapshot, the record) is headed by the same rule: prior working preferences guide method, style and decision defaults within the current handoff; current owner instructions and task-specific constraints win; preferences never create grants. A managed memory file that drifted from its managed revision is reported once per revision at session start with the `/memory accept` and `/memory restore` commands; see [docs/settings.md](docs/settings.md#context-memory).
