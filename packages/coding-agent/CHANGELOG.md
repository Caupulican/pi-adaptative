...
- System One judges the operations the deterministic gates cannot decide, for the root and for workers: code piped into an interpreter, a destructive command on an unexpanded variable, sending data off the machine (`curl -d`/`-X POST`, `scp`, `rsync host:`), a typed write outside the task directory. One batched request asks whether it leaves the machine, cannot be undone, touches files outside the task, and whether the owner's request asks for it; a local, reversible operation runs, an irreversible or outward one runs when the request asks for it and is refused when it clearly does not, and an unsettled one (or a System One that cannot answer within 8 s) goes to the operator as the new `operation.irreversible` edge class (a worker is refused). Verdicts are cached for the turn; ordinary work never reaches System One.
