...
- System One checks a plan when the model publishes or changes it (`task_steps` set, intake or add): whether a requested part has no step, whether a step needs a later step's result, whether any step checks the result. A decisive defect is appended to the tool result so the model fixes the plan before working from it; a likely one is shown as a doubt. Sessions without System One are unaffected.
- System One judges every shell or code call (`bash`, `run_process`, `powershell`, `python`) and every write outside the task, for the root and for workers; no pattern list decides which, since a model blocked on one spelling reaches for another. One batched request asks whether the operation leaves the machine, cannot be undone, touches files outside the task, acquires external code, and whether the owner's request asks for it. No likely effect runs silently; an established effect runs when the request asks for it, is refused when it clearly does not, and otherwise (or when System One cannot answer within 8 s) goes to the operator as the new `operation.irreversible` edge class, where a worker is refused. Verdicts are cached per turn; reads, searches, plan tools and writes inside the task never reach System One.
...
- `/usage` offers the Codex usage-limit reset whenever the owner is logged in to Codex, whatever model the session runs. When Codex answers that the usage limit is reached, pi says how many resets the account can redeem (a check that finds none or cannot complete is made again on the next limit answer), and a redeemed reset clears pi's recorded Codex exhaustion and usage-window limit so routing, workers and admission use Codex again at once.
...
- Under a handoff, a stronger model may settle an owner question on its own judgment when System One decisively finds the question touches none of the decisions the owner reserves (scope, spending, risk, irreversible or outward actions, taste).
- `npm run probe:system-one-ladder` measures the ladder's System One questions against live System One with cases of known answer and fails when any case settles the wrong way.
- System One allocates models for routed turns, workers and objective tasks. It judges which kind of model and thinking a task needs: one Choice over four categories (flash-light, flash-deep, strong-medium, strong-deep), each naming a tier. The owner's pin for that tier runs it in any selection mode, whenever it has auth, is allowed by the model policy, is not exhausted and can take the turn. Otherwise H-MoE picks inside the category from facts. System One judges which models are lightweight speed variants and which are superseded by a later version of the same model (Flash 3.8 over 3.6); flash models are ordered by measured turn time, first token included; a model the probes graded unfit sorts last. Images and context size are hard facts no choice overrides. Below 0.90 confidence, the question narrows to the two leading options and decides there. `npm run probe:system-one-route` measures this on the machine's real models.
- The owner turns model pools on and off by saying so: "we can only use subscription models", "enable the API models again and turn off the subscription ones", "no local models". System One reads the change (one yes/no question per turn, the pool questions only when it may apply; below 0.90 a narrower follow-up, below 0.80 nothing changes and the doubt is shown). Every allocation reads the policy when it picks, so the next turn, worker, lane or consult follows it: a pin or a session model outside it is reallocated from the allowed pool, even with the router off. A change that would leave no model to allocate is refused with the reason. System One itself is not a pool.
- Read-only workers get System One. `typesafe_review` needs the new `semantic.judge` capability, which a read-only grant keeps, instead of network and credential authority, which it strips; reviewers and auditors had silently lost the tool.

### Changed

- Model routing is on by default when the session has System One (a TypeSafe or OpenRouter key) and off otherwise; an explicit `modelRouter.enabled` still wins either way. Router configuration warnings are reported after the session starts, so they follow the same default.

### Fixed

- Calling an API or sending data with `curl`/`wget` is no longer an external acquisition: the objective-mode acquisition screen matched any use of those tools and replaced a request the owner explicitly made with "build the minimal local capability". A download of a script, archive or package file, a fetch piped into a shell, and a package install still are.
- An Antigravity rejection says why: the service's status and message follow `Antigravity inference failed (HTTP 400)` instead of being dropped.
- Boolean questions sent through the session's batched-question engine (evidence retention, the operation gate, plan review) keep their `criteria`; the adapter used to drop them, so System One never saw what a true or false answer meant.
- An objective transition System One cannot settle holds instead of failing: JEV-024 through JEV-028 (completion plausibility, primary completion, the cold challenge, delivery truth, release readiness) take the judged certificate whatever its outcome, and an outage or an ambiguity that outlived its gather-more budget ends the cycle as `semantic_gate_unavailable` naming the checkpoint (`jev_025_unavailable`, `jev_026_ambiguous`, …). JEV-025, JEV-026, JEV-027 and JEV-028 used to throw out of the loop, and the goal stopped as an opaque `blocked`. When the hold is an ambiguity (not an outage), the owner gets it as a question through the same host channel as other unsettled items (shown at once when the session is idle, else when the running turn ends; written to the follow-up document under a handoff), naming the check and its doubts.
- A worktree-discarding command (`git reset --hard`, `git clean -f`, `git checkout -- …`, `git restore`, `git stash`) asks the operator when the worktree's status cannot be read (a timeout, an oversized status, a lock) or the ownership check fails; an unreadable status used to count as "no other session's work" and the discard ran unasked. A directory that is not a repository still runs it without asking.
- Secret redaction before a System One request recognises a key by its body instead of a left boundary: a key glued to other text, or a second AWS key or GitHub token written straight after a first, is redacted; lowercase path words such as `task-automation-controller.ts` still are not.
- Routing and the session use only models the owner's accounts offer. At session start pi asks Codex which models the ChatGPT account may use and checks the OpenRouter key; before the first request a session model the account does not offer moves to the account's own default, and the router, workers and compaction never pick one. A model the provider still refuses mid-turn is marked for the session and the refused turn is re-sent at once. A routed turn continues on the tier's next usable model, else the root model, else the account's default, for that turn only: the turn's end restores the owner's model, and when nothing usable can take the turn it ends with the refusal instead of moving the session. A turn on the session's own model moves the session to the account's default. The Model Router status (session info and Router Setup diagnostics) lists what each account offers, a rejected key, or a check that could not complete.
- Git failures inside delivery checks no longer print `fatal: not a git repository` on the terminal; the message stays in the thrown error.
...
- Evidence the harness built wrong names its JSON path and is classified `invalid_request` without retries; `timeoutMs` bounds a whole System One evaluation.
...
- Secret patterns start at a token boundary: `task-automation-controller.ts` no longer reads as an API key and blocks the System One request that names it.
...
- The LLM routing judge is retired with its settings (`judgeEnabled`, `judgeModel`, `judgeThinking`), its settings rows and its fitness role; System One makes that decision.
- H-MoE's ranking no longer gives every local model a privacy and diversity bonus when locality was not requested, which made a small local model outrank every strong one for any work; its latency reads the host's measured speed instead of a field the perf profile never had; production passes the probe and measurement stores to its feature builder, which ignored both.
- A routed turn with an image goes to a model that reads images instead of failing on a text-only one.
...
- System One's Jev engines are no longer offered as chat models. The catalog marks TypeSafe's models (and OpenRouter's `typesafe/*` entries) `kind: "judge"`; the registry lists and allocates only conversational models, and still resolves an engine by id.
- A stored Antigravity catalog written by an older adapter is refreshed on first use instead of running models without their thinking budgets and backends until the next token refresh. OAuth providers can declare stored credentials stale (`needsRefresh`); a stale credential that is still valid stays usable when its refresh fails, and the upgrade is tried once per process.
- The model pools the owner turns on and off by saying so are kept on the session branch, so resume, fork and branch switches restore what was said on that branch.
- System One runs one engine version, Jev 1.13, on every path: the steering plane, the controller, model routing, the root's and workers' `typesafe_review`. Provider, model and key come from one resolver at every evaluation, so `/settings` > System One provider (`auto`, `typesafe`, `openrouter`) applies to the next evaluation without a restart. The review tool no longer sends one provider's key to the other provider's endpoint when its own key is missing, the steering plane no longer sends TypeSafe's model id through OpenRouter, workers review through the session's provider instead of TypeSafe only, and System One drivers no longer read keys (including a hard-coded `auth.json` path) beside the credential store. `systemOne.model` and the `systemOneModel` SDK option are gone.
- The owner is told once per session when System One judges a pinned model superseded by a later version among the authenticated models. The pin keeps running.
- Cancelling a running worker keeps its write reservation until the run has stopped; a queued worker needing the same paths no longer starts while the cancelled worker's in-flight edit or shell command can still write.
- Test fixtures and harnesses: the router red-team harness is built on the controller's prototype, the execution-realization fixture declares the model `input` and `cost` its provider contract requires, and the Calibrate menu test navigates by the cursor row instead of a fixed key count.
- System One's duplicate scan lists files with `/` separators on Windows as the find tool does; the scan found no candidates there.
- The Diagram view reads what is running from the flow trace instead of inferring it from phases: root's node shows its open action, a request without a goal ends as `turn finished`, `turn failed` or `turn cancelled` by how the turn actually ended, and stays `turn running` while anything in it (routing included) is still open.
- A collaboration member that fails its own pane acquisition, startAgent, readiness check or launchTurn is recycled on its own instead of rolling back the whole team: it is cleaned up and its launch retried once more, and, if it still fails, only that member ends failed while every other member keeps running and gets its turn. A failure that is genuinely team-scoped — an external cancel or dispose, an explicit stop racing the launch, backend/workspace acquisition itself, or the managed-workspace root member whose pane cannot be replaced without recreating the workspace every sibling was split from — still rolls back the whole team exactly as before. `CurrentPanePlacementStrategy` no longer advances its sibling-split anchor until a member's launch is confirmed, so a recycled member's failed pane never becomes the split point for the next one.
- A collaboration member left mid-stop by a process that died before finishing (a recycled launch failure, a team-wide rollback, or a stop whose own cleanup never completed) no longer stays uncertain forever, re-surfaced as the same unresolved notice on every future reload: session reconciliation now resolves it once the durable record proves nothing was ever created, or the live backend confirms the pane is gone or still exactly ours and closable, the same verified evidence the launch path itself requires. Anything it cannot prove either way — an outstanding acquisition, a live pane it cannot confirm, or an unreachable backend — is left exactly as uncertain as before; it never recreates native work and never closes a pane whose identity it has not verified.
- A clear `/goal` is no longer blocked at admission. System One's ambiguity score was read as its mean against a strict `> 1`, so tail mass on "significant" failed a goal it rated "minor, defaults resolve it" at 0.72; a Score is now read as the probability mass on the levels a check asks about, and at admission only a decisively incoherent objective fails (ambiguity and missing information gather more or ask the owner). The progress check (JEV-005) read the same mean and could practically never fire.
- A finished goal closes. The completion check gave System One the diff as paths and hashes and left out the goal's evidence, then required each of ten answers to clear the hard-pass edge; it now reads the patch from the last commit before the goal started (commits during the goal included, bounded, new files listed) and the evidence, and reads each answer against the ambiguous band's edges, where live System One separates finished work from unfinished.
- The acquisition gate's System One judgment runs: it sent an undefined `capability_id` and failed every time as an invalid request. It now asks over the owner's request and the exact command, with three of its questions phrased as defects (more than the request needs, runs an unverified download, the source provides something else) as measured live.

### Removed

- The model-fitness probe's LLM route-judge lane: routing is judged by System One, not a probed LLM call. `router_judge` is no longer a fitness-gated surface or a router calibration surface, `ModelFitnessReport` no longer carries a `judge` score, and the fitness store tolerates but no longer requires a stored report's legacy `judge` field.
...
- Recovery-log durability waits until the worker acknowledges the write. Windows directory cleanup keeps retrying a lock for several seconds. The RPC listener wait matches a slow session start. The goal-continuation interruption tests allow 90s so the provider retry backoff can finish on a loaded Windows runner; a missing stop still fails.
...
- Automatic commit records only successful typed edit/write paths, including worker writes on the same objective. A bash or run_process command marks `shell_mutation_unattributed` only when HEAD, the index, a tracked diff, or an untracked candidate changes, or when that fingerprint cannot be read. Ignored build output does not. An owned file whose bytes change without a new owned write fails `ownership_drift`. Lane bash still refuses `git add -A`/`--all`/`.`/`-u` without paths, `git commit -a`/`--no-verify`, stash, `reset --hard`, `clean -f`, `checkout .`, `restore .`, and any `git push`. Explicit-path `git add` and `git commit -m` stay available on a lane.
- Root bash admits no git invocation. Git reads use `repo_read` on the default tool surface. Commit, push, and tag stay on typed delivery. Lane git parsing is quote-aware and fails closed on an unfinished quote or a substitution.
- One repository mutation observer fingerprints every capability that can change the checkout, including Python, process tools, workers on the same git dir, and unknown tools while delivery is active. A typed write that also changes another path is unexplained. The candidate waits until those calls finish. The fingerprint streams file bytes. `shared_guarded` does not contain a destructive process; process-heavy dogfood stays on a disposable clone.
- The git tree certified for a commit is frozen before JEV-024. A later edit means that candidate is gone: completion runs again, freezes the new tree, and reruns JEV-024. An unowned path or a HEAD that left the admission baseline stays terminal. `completion_candidate` is read-only; a hook that asks to mutate does not pass. A push with no commit fails `stale_candidate` when HEAD is no longer the semantic candidate.
- Shared-worktree bash, Python, `run_process`, and toolkit scripts own a tracked process tree. The shell waits for job children, and a one-shot process group is empty before the tool returns. A tree that cannot be observed is `process_tree_untracked`. Command text is not a detach detector.
- `push tag <name>` pushes only `refs/tags/<name>` to the frozen remote and does not grant a branch push. The tag target is the proven commit, or the clean semantic candidate when no commit was requested. A tag is not proven without that target.
- Package publish is bound only from the charter's package intent. A public `package.json` does not grant it. The registry is frozen at admission. Publish packs once with scripts disabled and checks the registry `dist.integrity` against that tarball. Deploy runs only through a trusted adapter registered with the session; a mismatched deployed revision fails proof.
- Git commands that are given an explicit checkout do not keep a parent process's `GIT_DIR` or `GIT_INDEX_FILE`. A hook-hosted test can no longer stage that hook's index.
- The pre-commit hook starts every staged test without that hook git location, including tests that call git directly.
...
- Windows suite and agent-dir cleanup removes a tree with the same transient EPERM retry as an atomic rename, and the suite temp directory is the real path of the temp root. A short TEMP alias was leaving `rm` denied after the worker test had passed. The ownership check before that rename is not part of the retried operation. The long-directory prompt check compares the real temp path.
- The version-tag quality gate passes `full_suite: true` into CI. A reusable workflow keeps the tag push's event name, so testing `github.event_name == 'workflow_call'` skipped the matrix and provenance rejected the tag. Provenance reads that same tag workflow's quality-gate jobs. A reusable `ci.yml` call is not a separate `ci.yml` run.
- An output-limit abort no longer fails the process-group wait after the group has already exited, so `run_process` still reports `output_limit`. Project-rule checks read the session engine's `noul` probability, and a repeated identical stall is assessed again after a steer so it can reroute. Windows npm runs `npm-cli.js` with node; Node rejects `npm.cmd` without a shell.
- Foreground idle wait yields while an evaluation or armed continuation is still occupancy, instead of retrying on microtasks and starving that work. A running worker, a detached background tool, or the reflection turn itself does not block the next foreground turn, so a worker terminal, one background call, and the reflection turn can run. A later user prompt still cancels that reflection turn before it takes the lease. The goal loop reports `worker_in_flight` instead of waiting for that worker. An edit that ran and failed still names its path on the worker claim. An operator verification dismissal is written while a continuation is only armed. Lane bash does not treat `git` inside `echo` or `printf` as a git invocation. Clarification reads a JEV-001 `noul` answer. Preflight, tool-gate, and postflight read that same `noul` through `noulFromAnswer` instead of a missing `.boolean`. The Windows installer proves `pi.exe --version` from a copy of the staged tree, so activation can rename the staging directory. A Decision graph title that cannot fit `Follow current` keeps Hide and the view chips.
...
- Worker supervision is a first-class `JEV-WORKER-SUPERVISION` checkpoint with seven bounded judgments. The live session binds it to the SteeringPlane, not the compaction retention adapter. Invalid Jev/provider responses throw `JevAdapterFailure` with the original sanitized cause; they never become empty successful `answers` that a decoder reports as missing `meaningful_progress`. A successful reviewer payload that omits asked answers is `invalid_response` and is not retried. Missing supervision keys no longer default to 0 (silent continue). Unrecognized checkpoints fail closed instead of compiling generic `approved`. Observer failures are degraded, debounced, and do not abort the worker. Validation-churn steers once then reroutes. Specialist/capability/verifier requests are consumed by `composeObjectiveRoute` after workers are no longer in flight. Live observations include output tail, stall, and repetition.
...
- The Decision graph no longer lights `yes → DELIVER` while proof is still open, in both List and Diagram. Follow uses a semantic focus key, not only the current row number, so a reflowing diagram still tracks the active node unless the operator pinned the viewport. A pinned graph shows `new` when later progress lands off-screen. An in-flight Jev line is drawn once on the SYSTEM ONE box, not a second diagram level. Early-compaction hysteresis is measured from the last real early decision, not from skipped hysteresis looks.
...
- BLOCKER: the tool gate's `replan` verdict cancelled the whole foreground turn (System One's cancel lever), which killed every sibling tool call and the operator's own request and left an empty aborted assistant message; typing "continue" repeated it. The verdict now refuses that one call only and the turn goes on. The verdict itself was wrong on plain sessions: relevance was judged against an empty objective, so every call read as "not relevant"; relevance is now evaluated only against a real plan step or goal, and the relevance question is not sent to Jev at all when there is none (the decision record hashes the questions actually sent). Inside the objective loop a System One cancel of the root turn is a re-route, not a stop.
...
- System One drives the goal loop (`objective_primary`, the default whenever System One is bound; `systemOne.loopMode` overrides it): each continuation pass evaluates one objective route from deterministic facts and Jev's judgments and executes it as a root turn carrying the route brief (implement, investigate, replan, retrieval, non-independent verification), a worker (review, independent verification, escalation), a wait on the running workers, or the completion transaction (`systemOne.completionProfile`). The goal follows the objective's terminal and blocks, naming the unsatisfied requirements, when they disagree. Documented in `docs/system-one.md`.
- System One's levers over every executor: cancel fires immediately (the Esc path) and a steer is queued for the next model turn or delivered now; for workers, "now" interrupts the attempt, queues the directive and resumes it. The tool gate's replan verdict cancels the turn to re-route and its confirm verdict queues a scope steer; the worker supervisor redirects off-mission work now. Every directive is an operator event.
- The decision ledger records every route with its executor and feeds the objective's recent routes back to the judge as a history block; the stall evaluation is a fact from those rows.
- In primary mode the operator projection's CONTROL derives from the objective route itself: `owner_required` hands control to the user with the blocker, wait routes read as executing, verification routes as verifying, root-executed routes as executing only while the foreground turn runs. `owner_required` is produced from the session's operator blocker and a pending human-input request.
...
- The objective execution controller was handed the raw durable task runtime, which has no `reconcileObjective`, budget, repair or replan hooks, so every route evaluation threw on its first line and the shadow hook swallowed it: the route machine had never evaluated a real objective. A session-backed runtime folds the live goal into its durable objective; the shadow hook named the bare goal id where the objective is `goal:<id>`.
- The worker supervisor's steer was queue-only (`sendWorkerAgentMessage`), so it never reached a running worker nor woke an idle one; it now delivers through the session-root follow-up.
...
- System One CONTROL in the operator projection and POV bar: `OperatorProjection.control` (`owner` system_one | root | user, `state` deciding | executing | observing | verifying | awaiting_user, `reasonCode`, `clarificationRequestId`, `blocker`) is derived on every read from the goal continuation verdict, the durable human-input record, bound lane terminals and foreground activity — `CONTROL S1` while an active objective is owned by the goal loop (a returned worker bound to an open requirement shows `Reviewing worker result` and `review evidence`: worker done is evidence, not completion), `CONTROL ROOT` without an objective, `CONTROL USER` only while an owner question is pending, when the leading segment becomes `NEEDS INPUT <question>` with a `BLOCK` segment. Router and H-MoE are never control owners.
- Bounded owner clarification owned by System One (information only, never authority): an `ask_question` raised while a goal is executing is correlated to that objective on the durable human-input request (`objectiveId`, `category`) and in `GoalState.clarifications` (bounded ledger written by `clarification_requested` / `clarification_answered` goal events, settled on restart resume too), so a pending question survives compaction and restart and the answer resumes the same objective with CONTROL back to S1; the compact goal context shows recent clarifications as untrusted owner information. Before any dialog, `evaluateClarificationNeed` refuses duplicates (already answered or still pending) and, when a steering plane is bound, runs the JEV-001 sufficiency questions through the recorded engine — nothing missing means the tool returns a withheld result telling the model to proceed on its stated assumption; no engine means the ask proceeds and `semantic_unavailable` is recorded, never a fabricated verdict. Clarification changes no charter grant (`interaction_mode` stays `start_only`; a push or install mentioned in an answer is still denied) and workers cannot ask the owner.
- One operator POV bar in the Workbench (`WORKING | CONTROL | NEXT | ACTOR | ROOT | ACTIVE | ROUTE | JEV | COST | PROOF | CTX`) reading the live projection, the router's new `ForegroundRouteSnapshot` (root vs active model, route source), the semantic plane's observed health and the canonical cost summary on every render; width pressure drops context/proof/root-actor first and never cuts routing, Jev or cost. Documented in `docs/workbench.md`.

### Changed

- Router pool provenance is explicit: the candidate pool is session state separate from the model-cycling list, carrying its source (`enabled_models`, `cli_models`, `sdk_models`, `models_selector`, or `all_enabled` when uncustomized). An orchestration profile pins cycling to its root model but is never a pool source, so a profiled session routes across the whole Models configuration instead of a false one-model pool; pool summaries, the route preview and Diagnostics show the source, and Diagnostics/preview name a manual pin that resolves outside a customized pool (the pin still wins). SDK callers can pass `routerPool`; a `scopedModels` scope without one is the pool.
- Automatic selection and H-MoE ranking order the evidence class before the subscription preference: `known_fit` ahead of `unprobed` ahead of `known_unfit` (from the same FitnessStore lanes, whether or not the hard fitness gate is on), then `subscription-first` within the class, then the existing ranking. A known-fit metered model now beats a known-unfit subscription model; `subscriptionPreferred` is reported only when subscription decided within a class. H-MoE feature vectors carry `adequacyClass`; the ranking policy version is 1.2.
- Router Setup **Configure models** waits for the Models editor to close and reopens Router Setup on the edited pool (settings can open directly on one item's submenu via `initialItemId`); calibration copy derives its surface count and names from the canonical surface list and names the tool probe separately.
...
- "Calibrate unprobed" no longer skips a model whose fitness report exists but leaves a router surface `UNPROBED`; `needsCalibration` is derived from the per-surface states. A report missing the lane a surface needs now yields the unprobed verdict instead of throwing, and a changed registered context window marks evidence `STALE`.
- A session launched with an active orchestration profile (`pi --orchestration-profile`, including every worker resume, which spawns that flag) failed at construction with "remove conflicting SDK options: toolProfileFilter": the services adapter supplied a settings-derived tool-profile filter the profile owns, and the CLI also passed the model, scope and tools it had pre-resolved. The adapter now applies that default only without a profile, and the CLI hands the SDK the profile alone plus the router pool.
- H-MoE feature scoring read the worker lane for a judge request (the judge branch scored inline and was then overwritten); every branch now selects its lane and one block scores it, reading the judge lane's parsed count.
- A cancelled semantic evaluation (operator abort) no longer reports `JEV degraded`: the health recorder gains `recordCancelled`, which closes the in-flight mark without recording an outcome. The Jev indicator now records every in-session semantic evaluation (worker supervision, project rules, the acquisition gate and compaction retention all reach the plane's engine through one recording accessor), not only the compaction path.
- Foreground H-MoE model selection released no capacity lease, so after ten selections on one expert within the lease TTL the selector threw and refinement stopped silently (a live preview counted too); the foreground path now releases the lease as soon as it has read the ranking, and a failed selection is named in router status (`H-MoE: unavailable (…)`) instead of being swallowed.
- The route judge's spawned-usage id was derived from the prompt alone, so a preview followed by the same prompt (or a repeated "continue") was charged twice and recorded once; the id is unique per judge call.
- A failure while preparing a routed turn (settings read, thinking clamp, capability derivation, tool filtering or system-prompt build) left the session on the routed model with the routed-turn bookkeeping armed and a cheap turn's message buffer active; the whole preparation now runs under the restore.
- The execution charter's `git.force_push` grant is now evaluated: a force push (`forcePushRequested`, `force_push`, `git_force_push`) is denied with `git:force_push` unless explicitly granted, above the plain push check, so a push grant never authorizes a history rewrite.
- Research (specialist) lanes carry a bounded label from their demand, so the POV shows `specialist <label>` instead of `Worker <id>`.
- Two settings tests and two red-team router tests updated to the router selection-mode contract (`selectionMode` / `poolPreference` defaults; hand-built router hosts bind `isTierAutoSelected`).
...
- Execution Realization Closure (v1.6) with two-phase runtime binding (`AdaptiveRuntimeReadiness`), production-live port enforcement (`RealCapabilityBuilder`, `RealMechanicalVerifier`, `RealScriptRegistry`, `RealWorkerDispatcher`), fail-closed script activation, and dynamic Jev-driven specialty resolution.
- Release Freeze Master Bundle (v1.7) delivering evidence-preserving compaction (`EvidenceRetentionPlanner`) with diagnostic error pinning and paired call/result identities, semantic project rules (`SemanticProjectRuleController`) with AST/regex verification and automated `RuleRepairWork`, worker semantic supervision (`WorkerSemanticSupervisor`) with debounce and anti-oscillation, external capability acquisition gating (`ExternalCapabilityAcquisitionGate`) with deterministic hard-deny dominance and safe command rewrites, and operator state projection (`OperatorProjectionController`, `OperatorStatusComponent`).
- Gapless Release Candidate Readiness Closure (v1.7.1) making every frozen subsystem reachable from a normal SDK session: the capability builder requires a real worker execution owner, a resolved H-MoE provider/model binding, a real profile id and a real `WorkerResultContract` with matching lineage, artifact bytes and digest; capability proof obligations are compiled into runnable commands and executed through a bounded `CapabilityProofRunner` that records exit status, output digest and elapsed time; `EvidenceRetentionPlanner` is invoked by the real compaction flow and its decisions change the compacted branch; `SessionProjectRules` is consulted at mutation acceptance, task postflight and completion, queueing durable `RepairWork`; `WorkerSupervisionCoordinator` observes real worker tool calls and acts through the existing worker-agent control surface; the acquisition gate takes its authority from the actual `ExecutionCharter` and resolves a safe route to an executable plan; `SessionOperatorProjection` replaces the hard-coded TUI projection and the constant footer status; and durable owner development rules persist per project and reach worker, specialist and capability-builder missions.
- `ReleaseWiringManifest` with `npm run check:release-readiness`, a secret-free gate that verifies all 24 mandatory subsystems have a construction owner, trigger, consumer and negative-path test, plus a static forbidden-fallback scan whose own test proves each pattern rejects the shape it exists to reject.
- RC Activation Truth Closure (v1.7.1): every `CapabilityKind` carries an explicit support record. `ephemeral_script` (bounded execution of the artifact and its entry point), `extension` (extension-runtime load plus live registry lookup) and `runtime_patch` (the real snapshot/apply/verify/commit lifecycle) are owner-backed; `toolkit_script`, `tool`, `skill`, `composition`, `integration` and `provider_adapter` are explicitly unavailable, excluded from semantic selection, and have no activator at all. A spec naming an unsupported kind replans onto the lowest adequate supported kind or blocks, and `runtime_patch` is never an automatic replan target. Adaptive readiness reports the matrix and separates an unavailable optional kind from an advertised kind with no owner. Availability is resolved against the live session: `extension` requires project context files to be enabled, because the resource loader refuses project instruction paths otherwise, so a default session reports `ephemeral_script` and `runtime_patch` as the supported set. `verifyCapabilityKindActivationTruth` runs in `check:release-readiness` and the architecture audit, so a matrix advertising an owner it does not have fails the gate.
- `npm run audit:architecture` writing a deterministic, secret-free architecture audit artifact, and `npm run smoke:live-provider`, a local-only optional smoke that is never invoked by CI and never blocks a release.

### Removed

- The metadata-only capability activators for `toolkit_script`, `tool`, `skill`, `composition`, `integration` and `provider_adapter`, which reported `active: true` with asserted `registered`/`wired`/`adapterMounted` claims and constant `smokeEvidence` strings that no owner produced.
- Every fabricated production success path in the adaptive execution ports: the fabricated `WorkerResultContract`, the fabricated specialist result, the empty dispatcher delegate, the default `anthropic`/`claude-3-7-sonnet` expert binding, the fallback task-profile id, the generated `return true` capability source, the asserted `verified: true` task proof, the permissive acquisition authority defaults, and the hard-coded operator execution projection.

### Fixed

- `computeArtifactDiskDigest` resolves a `file://` artifact URI with `fileURLToPath` instead of stripping the prefix, so a percent-encoded path segment (a Windows `RUNNER~1` temp directory arrives as `RUNNER%7E1`) or a leading slash before a drive letter no longer makes an existing artifact look absent.
...
- Specialist allocation records preparation identity before task creation, allowing recovery before transcript binding and cancellation after a lost preparation receipt without leaving abandoned work queued.
- Registered, never-started workers resume their accepted queued task after a proven-dead owner exits. Competing controllers cannot cancel another owner's enrolled task during dispatch or shutdown.
- Reusing a Herdr specialist after resuming the same parent session in another process refreshes supervision ownership and rejects stale controller writes.
...
- Owner working preferences (USER.md) are learned with evidence and delivered where decisions are made. Each root-session owner prompt is recorded as owner evidence (an `owner_evidence` session record holding the owner's original words, bounded and digest-verified, never the expanded text; rebuilt after restart, unaffected by compaction) and listed by id in the reflection cue. A `memory` write to target `user` names its scope (`global`, or `project` for this repository), its basis (`explicit` or `inferred`) and its evidence (`[{ source, quote }]`); only a citation whose quote is found in the owner's own words outside shown text counts (a quote inside quotation marks, backticks, a fence or a blockquote, a worker report or a tool result does not; a citation without a quote does not), evidence not newer than the accepted line's own evidence time (`at=` in its trailer, written on commit) is a replay and does not, and counted receipts are stored per scope and value (the eight newest per value) so independent observations accumulate across sessions while a replayed source stays one; a scoped preference's identity includes its project, and a write supersedes, replaces or removes only a line of the scope it names, matched against the archive as it is at write time. The learning gate then admits the write: an explicit, supported preference or correction applies at once, also over an explicit or legacy line and even with automatic learning disabled; an inferred pattern needs two independent owner observations and never overrides an explicit or legacy line; anything else is a durable learning candidate (audited `learning_audit`/`learning_decision` records, no approval prompt). An admitted write is audited only after the file-store reports the commit (`apply` with a rollback plan) or its failure (`apply_failed`). The accepted line carries one human-readable trailer (`[pref <id> | global|project=<key> | explicit|inferred|unverified | n=<sources> | rev=<n> | at=<time> | src=<ids>]`, sources of the current value only) written in the same USER.md/archive transaction; legacy lines stay usable and unannotated; `expectedRevision` refuses a stale update. Every projection (frozen root block, `user_persona` record, worker `memory_read` snapshot) selects whole applicable lines within the USER budget, rendered as `- text (explicit|inferred, n independent observations)` with project scope checked by directory key, a legacy line keeping its USER.md heading path as context, applicable archived preferences included and omitted lines counted.
- Handoffs receive applicable owner working preferences upfront: in-process workers through the worker system prompt (`personaGuidance`, composed per attempt so a persistent worker's next task carries current guidance) and Herdr teams through the shared `fire_task` objective and every `send_followup` task, composed by the memory controller from explicit, well-supported inferred and legacy lines within a fixed bound that includes its own framing; never a grant, never MEMORY.md, OKF or recall. Extensions can read the same text through `pi.getHandoffPersonaGuidance()`.
- USER.md mutations reach the next provider request as one `user_persona` record (bounded to the memory prompt budget in whole lines, recorded once per revision, carried across compaction, explicitly cleared when the file and the installed block agree again or memory is disabled). Every USER.md projection is headed by one rule: prior working preferences guide method, style and decision defaults within the current handoff; current owner instructions and task-specific constraints win; preferences never create grants.
- Memory providers may implement `onSystemPromptBlockFrozen(renderedBlock)`; `MemoryManager.freezeSystemPromptBlock` (the system-prompt builder's call) invokes it with the block as installed, so a provider measures later mutations against what the model actually sees.
...
- A managed memory file (`MEMORY.md`, project `MEMORY.md`, `USER.md`) that differed from its managed revision at session start locked every write to that target out silently: the refusal was visible only inside the model's tool errors. The file-store provider now reports drift at initialization through the session warning path, once per active on-disk revision per file (a reload repeats nothing; a different revision, including a return to an earlier one, is a new fact), naming both digests and the `/memory accept <target>` / `/memory restore <target>` commands. A warning raised before any subscriber exists (SDK sessions initialize memory before returning) is held by the startup-warning owner and delivered to the first subscriber. Drift fencing is unchanged: the on-disk bytes are kept and rendered, nothing is adopted, and writes stay refused until the operator decides. Child sessions report nothing. `FileStoreProvider.drainHealNotices()` became `drainManagedNotices()` and returns structured notices (target, kind, revision, message).
...
- Machine-wide provider admission (`core/provider-admission/`). Every pi process on the agent directory registers each in-flight provider request under `state/provider-admission/` (one file per request, released when its stream settles, pruned by any reader once its owner's pid is gone or its heartbeat is stale) and shares what it learns about the accounts it uses: the first process to receive a 429 or overload with a known reset (a `retry-after` header, a stated delay, or the delay its own retry ladder chose, the owner's and a worker's alike), or a fully used Codex subscription window, records the reset under `state/provider-admission/limits/`; every lane consults it before sending, waits out a limit that fits its budget and otherwise refuses the request unsent with a message the reliability classifier reads as a rate limit carrying the remaining delay, so no retry ladder rediscovers a limit at the account's expense. Nothing invents a cooldown. A success clears a rate-limit or overload record. Codex window snapshots are persisted under `state/provider-admission/usage/`; billing exhaustion is shared under `state/provider-admission/exhausted/` with model or provider scope. Limits and counts are keyed by provider plus a non-secret credential identity (`<provider>#<identity>`), so two accounts on one provider are two budgets. The owner's foreground lane is never held for capacity or the emergency stop; worker and background lanes wait at `providerAdmission.limits` (no provider is capped by default) and while the stop is engaged. Waits are durable `provider_admission` entries and live `provider_admission_wait` events the activity lane renders (which lane, which provider and account, why, the reset countdown, the admission). Settings: `providerAdmission.{enabled, limits, maxWaitMs, foregroundLimitWaitMs}`.
- `/load` shows machine-wide in-flight requests by provider, account and lane, recorded limits, the latest provider windows and the emergency stop. `/estop on [reason]` writes `<agentDir>/ESTOP`, which holds new worker and background provider requests in every pi on the machine (queued workers stay queued, a running worker's next request waits then refuses) while foreground turns continue; `/estop off` lifts it.
- Fresh, unpinned workers run on another account than the foreground (`workerDelegation.account`, default `other`): the first candidate of `workerDelegation.routeProviders` (`provider` or `provider/modelId`), or of `routeProvidersByRole` for the worker's role, then every other provider with a stored credential in catalog order, skipping the foreground's provider, exhausted models and accounts with a live machine-wide limit; with no alternative the worker inherits the foreground. Automatic candidates are accounts only: a provider that merely needs no auth is never chosen unless named. An authority model, a model pin and a profile binding are never moved.
- Workers run one thinking notch below the foreground level when nothing pins theirs (`workerDelegation.thinking`: `step_down`, default, or `inherit`); `minimal` is the floor and `off` stays off. Authored levels are never moved.
- Every automatic provider retry is a durable `provider_retry` session record (start: attempt, backoff, failure; end: outcome), joined to its `request_snapshot`. Worker conversations write a `request_snapshot` per provider request, so a worker's request start and reasoning level are recoverable from its own transcript.
- `scripts/multi-tenancy-census.mjs` reports first-token latency by machine-wide in-flight count, retries by model and reason and admission waits by reason over sessions and worker conversations, and gates on two processes rediscovering the same rate limit within five seconds (`npm run check:multi-tenancy-census` runs its tests).

### Changed

- `workerDelegation.maxConcurrent` defaults to 3 (the Codex CLI's per-session default of four threads including the root), was 20.
- Extension `registerTool` rejects a tool whose parameter schema is not an object at registration instead of breaking provider request serialization later (hand-ported from upstream).
- Plain-HTTP origins stay on the proxy's CONNECT tunnel after undici 8.7 changed the default (hand-ported from upstream).
...
- CI: the Windows shell corpus wall (over a minute on Windows) runs by itself after shard 1 instead of beside second-scale budgets in a parallel shard; three tests had failed on time alone while it ran next to them. The compaction-stall, dependency-dispatch and iconv-recovery tests wait on their file budgets rather than arbitrary sub-second or five-second caps.

## [0.99.18] - 2026-09-11

### Changed

- A turn the host starts for itself is requested one thinking rung below the session level. The request that answers a `background-tool-completion` or `background-worker-completion` message has one job: read the delivered result, cite it, continue; it used to run at whatever effort the operator had chosen for their own questions. The drop is floored at `low`, never goes above the session level, skips a rung the model does not offer or maps to the same provider effort, and leaves the session's own level (and `/thinking`) untouched. `reasoning.hostTurnThinking` takes an explicit level, clamped to the session level, or `"inherit"` to keep the previous behaviour. `/usage` reports the last decision and how many requests this session lowered.
- The interactive live row says where a turn's time went. Until the provider's first token the turn reads `(9s, no token yet)`, after it `(23s, first 9s)`; on a fast provider (first token under three seconds) the row is exactly the elapsed figure it always was. The mark uses the same first-token definition that stamps `AssistantMessage.firstTokenAt` and splits the model perf profile, so the row and the telemetry can never disagree, and it belongs to the turn: each new turn starts unmarked.
- `edit` and `write` refuse U+0000 before they touch the filesystem. A replacement or a content string carrying a NUL is a transport defect, not content, and once written git reports the file as binary while the byte renders as nothing in a terminal. The refusal happens in the validation phase, before the mutation queue, before the file is read and before a parent directory is created, and names the 1-based edit index, the character offset counted in code points and twenty characters of context on each side with the NUL escaped as `\0`. It asks for the same replacement re-sent without that character and never repairs the string itself; `edit` skips the check when the edit's own `oldText` also carries U+0000, leaving a NUL-bearing source to the edit encoding contract. Repair codes `nul_in_replacement` and `nul_in_content`.
- The mutation barrier's group lock and its emission-order announcements are now per session instead of per process: each agent session (and each worker lane) gets its own scope, keyed by its shell session identity, so one session's exclusive command run no longer parks another session's file writes and one session's batch switch no longer retires another's announcements. The per-path mutation queue stays process-wide, so two sessions writing the same file still take turns.
...
- A background task's completion wake-up carries each finished record's bounded final output verbatim, in emission order, under a 24 KiB message budget; a record that does not fit is marked `output omitted` with the exact `tool_task wait` that collects it. Delivered records count as observed, so the extra `wait` request that used to fetch bytes the record already held is gone; the handoff stub, `tool_task` guideline and `background` descriptions now say wait only for an omitted output.
- Goal tool evidence matches a cited command on what was executed (whitespace layout and one trailing `;` normalized, everything else exact; python `code`/`scriptPath` now match too), and a miss names the newest producing call ids with an excerpt each. `add_evidence` accepts `requirementId` / `requirementIds` and satisfies them in the same call once the evidence verifies. A directory cited as file evidence is refused naming a file inside it.
- Stream-stall budgets are split by model class. `retry.stall.local` governs local and pi-managed models over the existing generous defaults; `retry.stall.cloud` governs every hosted provider over new cloud defaults (connect 120 s, active idle 120 s, quiet idle 300 s). The legacy top-level `retry.stall.{connectMs,activeIdleMs,quietIdleMs}` keys are the local budget; while they are set without a `cloud` entry, the settings diagnostics report the narrowed scope once at startup. Validation names the exact key written.
- A stored OAuth credential that expired and could not be refreshed is reported as such (`OAuthCredentialUnusableError`: provider, expiry, redacted reason, `Run pi login <provider> to reauthorize`) instead of `No API key for provider`; fallback key sources never stand in for it.
- Exported variables are shared across a session's bash lanes: each lane reports its `export -p` listing in the command sentinel only when it changed, the session's export ledger merges the delta that lane produced (set, changed, unset), and a lane behind the ledger replays what it lacks before its next command. `export FOO=1` in one command and `$FOO` in a later one now agree whichever lane the pool picked. Windows engine lanes already shared one state; the PowerShell floor stays lane-local.
- The mutation group lock is keyed by worktree (`worktree:<realpath>`), so a worker lane working in the parent's tree interlocks with the parent's command runs and writes again; emission-order announcements are per announcing session inside that scope, so a wave switch or an index comparison never crosses sessions. Scopes are refcounted by the sessions and lanes constructed in them.
- A settings diagnostic recorded by both the startup and the runtime SettingsManager (the legacy `retry.stall` note, worker-delegation notes) is printed once, under the first context that produced it.
...
- `tool_task action=list` no longer swallows a finished task's result. The listing prints only `taskId: status - summary`, yet it marked every terminal record it showed as observed; since the completion wake-up delivers only records still unread, a list that happened before delivery made the wake-up skip exactly those records and their output never reached the model. Listing is now a status-only read: delivery is consumed by the wake-up that carries the output and by `tool_task wait`, never by a listing, and the listing says so. The wake-up also measures itself now: every record in its details carries `outputBytes`, the UTF-8 length of that task's final output, and `inlined`, whether this message carried that output in full, so the 24 KiB message budget can be priced from delivered evidence that persisted records (which drop their output) no longer hold.
...
- `goal add_evidence` no longer fails when ledger bookkeeping lands while its verification awaits: an `increment`, requirement change, dispatch, progress tick, or budget record in the same batch is replayed under the existing ancestry proof. Lifecycle and objective changes still require a fresh observation.
...
- `scripts/turn-preamble-census.mjs`: per turn kind (root prompt, ping, goal continuation, reflection, wake) the bookkeeping calls a model makes before its first piece of real work, attributed to the directive each answers to, so a prompt change is measured before and after instead of guessed. Read-only; no prompt changed with it.
...
- Steering delivers together: `steeringMode` defaults to `"all"`, so every queued steer reaches the next model turn in one delivery; with `"one-at-a-time"` the live row says `N steering → one per model turn` instead of implying they all go.
- The workbench hint row shows the follow-up gesture — `>> follow-up` on Windows Terminal, which swallows `alt+enter`, and the key elsewhere — and Execution retains `workbench.previews` previews per cycle (24 by default, 4..200; was a fixed 12) for reading a long cycle without the transcript.
- On Windows, a CRLF script is normalized to LF before routing on both tiers, so a `\r` never reaches a coreutil as an argument (`sort: cannot read: /r`); a cmd.exe builtin that is not found (`dir /b`, `type`, `findstr`, `del`…) names its bash spelling in the refusal.
- Workers that can read. A parent with `bash` now lends `grep`, `find`, `ls` and `repo_read` natively (read authority, never `bash` in their place), and a parent that may run processes lends `repo.read`; a `readOnly: true` worker therefore gets `read, grep, find, ls, repo_read` (plus `skill` and memory query when active) instead of `read` alone. The census had 37 delegate failures on exactly this: `readOnly` excluding `bash`, `orchestration_tool_capability_missing:bash|python`, and `orchestration_tool_unavailable:grep,find,ls` on bash-only parents.
- Delegate spellings with one reading are absorbed instead of refused: `task` on `follow_up`/`send` is the message, `agentId` on `status`/`review` resolves to that agent's latest lane (the reply says which), `message` on `interrupt` is queued for the paused worker and delivered by `resume`, and `retire` with undelivered control messages lists them and accepts `force: true` to discard them. Plural ids on singular actions and two selectors at once still refuse.
...
- "History hidden for typing performance" no longer appears on a fresh session: the placeholder counts conversation turns (user, assistant, answered questions), not the control records every session starts with.
- Managed memory no longer locks itself out. The managed state stores the committed content beside its digest; a managed file found empty on disk (truncated outside the protocol — the census saw eight sessions refused on the empty-file digest) is restored from the managed revision at start and before a write, with a notice. Real external edits still refuse the model's write, and the operator now has the two ways out the old text promised and nobody could perform: `/memory drift` reports each file, `/memory accept <memory|project|user>` adopts the on-disk content as the managed revision, `/memory restore <target>` brings the managed content back and keeps the drifted file as a backup.
...
- The Windows shell corpus tool (`scripts/windows-shell-corpus.mjs`, `pi-shell-engine/corpus.py`): the harness harvests every `bash` call from its own session transcripts (this machine or an extracted archive from another one), sanitizes each into a grammar-preserving shape behind a hard leak guard, records the real command's grammar verdict, replays the corpus through the router, the grammar and the executor with real GNU tools, and reports defects; `--write` appends new shapes to the corpus fixture. The wall test and the tool share one replay engine, and the committed fixture is regenerated by the tool.
...
- Expanded `p/` path aliases in tool arguments to absolute paths under the legend root. The loop prepares arguments before any directory admission exists, so a repo-relative expansion handed to a tool admitted into a pinned directory was re-resolved there (`packages/coding-agent/packages/coding-agent/…`) and every aliased `read` after `task_directory select` failed; display expansion keeps the relative spelling.
...
- Kept encoded reads in one isolated Python helper with bounded, correlated frames instead of spawning a helper per chunk. Early return, cancellation, source I/O failure, pipe errors, and malformed responses close the read session; final success requires a matching process exit.
...
- Search output through bash is reduced to the grep tool's layout: `rg`/`grep` results (`path:line:text`, context lines, group separators) regroup under one path line per file with `  line: text` rows, capped at 60 hits per file (30 on the compact level) with the remainder counted; output modes without `path:line` (`-l`, `-c`, `--json`, `--files`, no filename, no line number) and explicitly verbose commands pass through, and any line the parser does not understand leaves the whole output untouched. The raw output is persisted before the model sees the shorter version, the result ends with one notice naming the raw path, and `details.outputReduction` records bytes in and out for the census. Replayed on a 14-hour live session it saves 25 % of rg bytes (65 % on single-file results).
- Transient records survive compaction: when the session context is rebuilt after a compaction, the current record of every append-on-change kind that fell before the cut (skill context, goal context, memory evidence, the failure ledger; every delta of a cumulative kind such as the path-alias legend) is carried forward right after the summary, unless the kept tail already holds a record of that kind. The reconciler therefore finds each kind's last content and appends nothing on the first request after compaction. Measured on a live session before this change the skill context was re-sent 75 times for 7 distinct contents and the legend 256 times, almost all of it on the request after each of 82 compactions.
- JSON output through bash and python is reduced to its shape and its anomalies: a document over 2 kB keeps every object key, arrays longer than six items keep their first three, their last one and every item carrying an error or a non-ok status with one `[… N more items]` marker where the cut is, and strings over 240 characters are clipped with their length. Pretty stays pretty, compact stays compact; the raw document is persisted and the notice ends with a jq projection derived from its shape (`project it with: jq -c '.items[] | {id,status}' <path>`), so the omitted records are one jq call away instead of a full read. jq is a pinned managed binary on every platform; the bash description and the prompt guideline now steer JSON through a jq projection up front (measured on a live session: zero jq calls, JSON dumped raw every time).
- `read` gains `mode: "outline"`: one `line: declaration` row per function, class, type, method or heading (TypeScript/JavaScript, Python, Rust, Go, C#, PowerShell, shell, Markdown; unknown types return the first 40 lines with a note), so a large file is oriented in a fraction of its size before the model reads the range it needs. Verbatim reads are untouched. Measured on a live session 75 % of tool-result bytes were reads and 85 % of those were re-reads of files already seen, scrolled in 70 to 260 line chunks.
- The capability tier carries the output-reduction level: frontier and strong tiers reduce at the standard level, the constrained tier at `compact` (lower per-file hit caps, one framed error per message, repeats collapse at two). `toolOutput.level: "auto"` follows the tier; the feature ledger gains an output-reduction row (results reduced, bytes in and out, share saved) and the doctrine gains the reducer contract.
- Every `bash` and `python` result now passes through one output-reduction pipeline, not only the recognized families. A generic stage runs first on any command: ANSI escapes stripped, carriage-return progress frames resolved with terminal overwrite semantics (only what the terminal would have shown survives), trailing whitespace and blank runs collapsed, three or more identical consecutive lines collapsed to one copy plus `[line repeated N times]` (two on the compact level). Then the first family reducer (search, diagnostics) or data-driven rule that applies runs on the cleaned text. Rules live in `.pi/output-filters.json` (project), `~/.pi/agent/output-filters.json` (user) and a bundled set (npm/pnpm/yarn install, pip install, docker pull/build), each with inline tests that `npm run check` executes (`scripts/check-output-rules.mjs`); a defective rule file fails tool creation with the file and rule named. Recovery is spent only when needed: the raw output is persisted and the notice added when a reducer reshaped the output or the generic stage dropped lines and the cut is at least 512 bytes; pure cleaning writes no file and says nothing. The model can bypass every stage per call with `fullOutput: true`; the operator with `toolOutput.reduction: "off"` (or `PI_TOOL_FILTER_DISABLED=1`), and `toolOutput.level`/`toolOutput.rulesFile` tune the level and add rules (`docs/settings.md`).
- GPT-6 Astra is selectable on the OpenAI provider (`openai` / `gpt-6-astra`, list prices with the 272k long-context tier, reasoning low through max) and on the Codex provider (`openai-codex` / `gpt-6-astra`, Responses Lite, low through ultra); see the ai package changelog for the catalogue details.
- The lean capability profile's stable system prompt budget is 10,240 characters (about 2,500 tokens, 15 % of the 16k window that defines the class) with a 3,584-character tool-guideline ceiling, up from 8,192 and 3,072. The old envelope left under 100 characters of margin once tool schemas and a Windows runner path were in place, so every guidance change overflowed it on one platform.
- The diagnostics reducer is language-agnostic: besides the rustc, tsc, biome and eslint/stylelint parsers it recognizes the location grammars nearly every compiler and linter prints (GNU `path:line:col: severity: message` for gcc, clang, mypy, pylint, htmlhint; MSVC and Free Pascal/Delphi `path(line,col): error C2065:` / `unit.pas(12,5) Error:`; Go `file.go:12:5: message`; PHP `Fatal error: … in path on line N`; ruff/flake8 `path:line:col: F401 message`), and it applies to any command whose output carries three or more such lines (`make`, a build script, a python run), not only to known tools. tsc's `--pretty` layout is parsed too, and when the same message repeats across files the report is laid out by message with one position list per file (`error[TS2322]: Type 'string' is not assignable to type 'number'.  (24 places)`). Python tracebacks keep the exception, the first frame and the last three frames with a count for the middle, on bash output and on the python tool's stderr. Replayed on synthetic reports: tsc --pretty 80 %, ruff 69 %, Free Pascal 67 %, tsc 63 %, go build 61 %, gcc through make 55 %, tracebacks 72 %; eslint's stylish table is already dense (26 %). Every fixture is a corpus gate.
- Compiler and linter output through bash is reduced to grouped diagnostics: rustc (through `cargo check`, `build`, `clippy`), `tsc` and `biome` reports render one line per distinct diagnostic under its file (`line:col severity[code]: message`; the same message at several positions becomes one line listing them, `warning[unused_variables]: unused variable  at 42:9, 57:9`; the lint name is recovered from rustc's `#[warn(...)]` note), code frames survive only for errors and only for the first two instances of each distinct message within a budget of eight, progress lines (`Compiling`, `Checking`, npm progress) and tsc's errors-per-file table are dropped, and every summary or unrecognized line is kept verbatim in place. Raw output is persisted and named in the notice as for search results. Replayed on a live session a 34 kB `cargo check` with 81 errors of two kinds shrinks to about 7 kB (79 % saved); on the fixture corpus a check with 30 warnings and one error shrinks by 73 %, a biome lint report by 68 %; the check gate holds both at 60 %.
- Command classification looks through shell wrappers and past redirections: `powershell -NoProfile -Command "Set-Location D:\\repo; cargo check 2>&1"`, `bash -lc "cd /repo && rg …"` and `cmd /c …` classify as the program inside (its `cd`/`Set-Location` becomes the working-directory prefix), and `2>&1`, `> file`, `< file` no longer make a command unclassifiable. Measured on a live Windows session every cargo, git and rg call went through such a wrapper and no reducer had reached them.
- Output-reduction census: `scripts/output-reduction-census.mjs` reports, per session, where the tool-result bytes go (by tool, and for bash and python by command family), which results a reducer touched and what it saved, and the passthrough families that carry the most untouched bytes. `--replay` runs the bundled reducers over the recorded raw outputs to project what a change saves before it ships; `--corpus` replays a synthetic fixture manifest (`test/fixtures/tool-output/`) and `--gate` fails when a family's reduction falls below a floor. The classifier is the runtime's own (`command-family.ts`: cd prefixes and environment assignments folded away, package runners unwrapped to the program that produced the output, explicit verbose flags flagged), so a family reported as passthrough is one the pipeline really left alone.
- The git filter and the test-output projection run in the live agent again. The runtime always installs an environment-only spawn hook on the bash tool, and the tool treated any hook as an execution override that disabled both, so every `git diff`, `git show` and test run reached the model raw and test-evidence verification was never recorded (measured on live sessions). Family filters now key on the resolved spawn context: a hook that only adjusts env or cwd keeps them on and the filtered git run receives that env; a hook or command prefix that rewrites the command turns them off for that call; projection and verification never depended on the hook.
- The git filter accepts what the model actually types: `cd <dir> && git …` (the `cd` is replayed into the persistent session first, and the filtered run happens where it landed, so the next command sees the moved directory), `-C`/`-c` global options, and a final `| head -N`, `| tail -N` or `| cat` stage applied to the filtered output. Expansions, globs, redirects and other pipelines stay with the shell. Measured on live sessions the classifier accepted 8 % of git calls; the accepted shapes cover the other 92 %.
...
- Project-scoped hot memory: the global `MEMORY.md` holds facts true in any task (budget 1,200 chars); each project gets `memory/projects/<key>/MEMORY.md` under the agent home (budget 2,200), keyed like OKF; the `memory` tool's `target` gains `project` and defaults to it; the rendered block shows the general, project and user sections; an over-budget general file adds a triage note; nothing is moved or deleted automatically; workers read project memory and never write it. An `okf_organize` reflection write finds its source line in either hot file (the engine reads both rendered sections), removes it from the file that holds it, and a rollback restores it to that same file.
- Context GC with the sent-prefix mark carried across runs: nothing below the mark changes spelling between two grid crossings of the quantized recent boundary; at a crossing the batch that aged past the boundary the previous request packed against rewrites as one unit, below the mark included, and a message packed once keeps its packed form while frozen instead of reverting to its original. Deep supersessions (an older read of a re-read file) join a crossing batch once they save `contextGc.deepPackMinTokens` (default 8,000). Rewrites therefore land once per stride at the tail, never per turn and never because a run started; the long-session contract gate counts rewrites against the number of grid crossings.
...
- Goal evidence of kind `test` verifies through the tool call that ran it, like `tool` evidence; the `uri` description names the locator each kind needs, and an unverified entry says why it did not verify and what would. A test run recorded with a command string as its locator could never verify, and the model then failed `satisfy_requirement`, `complete` and `increment` in a row.
- Host records (memory evidence, goal context, skill context, the failure ledger) are opaque to path aliasing, so their bytes never change between the request that introduces them and the next; aliasing a skill record's base path once it was history invalidated the cached prefix.
- The Codex transport telemetry names why a request went out in full instead of as a delta (`deltaSkipReason`: `no_continuation`, `shape_mismatch`, `input_prefix_mismatch`, `no_response_id`).
...
- Path aliasing no longer turns a sentence into a filename. A candidate containing spaces needs a trailing file extension as evidence that it is a path, but the extension could come from a later sentence — `Use extensions/packages only. npm and Node.js are supported.` matched across the sentence break and aliased as one `.js` filename. Evidence now has to belong to the candidate as a whole, so a period followed by whitespace disqualifies it, while legitimate spaced filenames such as `artifacts/reports/release notes (final).md` still alias. The punctuation-free ambiguity is left explicit and pinned by test: `Use extensions/packages only with Node.js` is lexically identical to a valid unquoted relative filename, and rejecting it would reject that supported class too.
- FFF native search recovers an installation that was left without its staged `ffi-rs` binding. A managed install completed by an older release staged the binding only after installing, so an upgraded tree could hold a complete package whose binding never got staged, and the first load failed before any repair ran. Staging now happens before the first load attempt and before the offline and provisioning gates. It is opportunistic: a corrupt or unreadable managed tree cannot block the other load roots or turn an offline closed fallback into a bootstrap exception.
- `tool_task list` shows why a background task failed instead of truncating the reason away. A failed task's summary was the raw `[harness]` record cut to a fixed length from the front, and the harness's own bookkeeping (`MUST`, `occ`, `kind_mistakes`, `mistake_kind`, `failure_key`) sits ahead of the cause — so the listing ended mid-key at `…"failure_code` with the value beyond the bound. The summary now projects the model-facing fields in cause-first order and drops the bookkeeping.
- An elapsed `tool_task wait` reports how long the task has been running instead of only telling the model to continue independent work. When the waited task is the session's sole dependency there is no independent work, and the turn was spent restating that the task was still running.
...
- `--` ends option parsing, so everything after it becomes prompt text or an `@file` argument. A prompt beginning with a dash previously errored as an unknown option with no way to escape it.
- `/thinking` sets the thinking level directly (`/thinking high`) or opens the level selector with no argument, validating against the levels the current model actually supports and naming them when the argument is wrong. Both the model and thinking selectors now also accept Ctrl+S to persist the highlighted entry as the default; plain Enter stays scoped to the session. This is a behavior change worth noting: selecting a model with Enter no longer writes it to global settings.
- `defaultTools` in global settings seeds a session's tool allowlist, in the same format as `--tools`. It filters the construction-time tool set rather than adding to it, so it can never re-admit a tool that the worker capability ceiling removed, and it is ignored entirely for worker sessions and orchestration profiles.
- RPC gains `clear_queue`, which drops the queued steering, follow-up and command messages and returns their texts so a client can restore them into an editor, and `get_available_thinking_levels`, which reports the levels the current model supports. `toolcall_start` in the JSON and RPC event streams now carries the tool call's `id` and `toolName`; every pre-existing field is unchanged.
- Extensions can register a display-only markdown transformer, chained in load order and applied to user, assistant and thinking text as it renders. A throwing transformer is skipped rather than breaking the render. Session history and the provider request keep the original text, and path-alias expansion runs before a transformer sees anything.
- Extensions receive `ui_prompt_start`/`ui_prompt_end` around `ctx.ui.*` prompts, with nested prompts coalescing into a single span and handlers dispatched fire-and-forget so a slow handler cannot delay the prompt a person is waiting on. A `session_compact_failed` event reports compaction failures with their reason, abort state, retry intent and whether an extension produced the summary. `agent_end` now carries `willRetry`.
- `pi auth check` reports whether a provider or model has usable credentials, refreshing an expired OAuth token unless `--no-refresh` is given, with `--json` for scripting. It reports status only and never prints secrets.
- Terminal capabilities can be overridden through `terminal.hyperlinks`, `terminal.images` and `terminal.trueColor` in settings, or `PI_HYPERLINKS`, `PI_IMAGE_PROTOCOL` and `PI_TRUE_COLOR` in the environment, for terminals that are detected incorrectly. Settings take precedence over the environment, which takes precedence over detection.
- Shell processes receive the live session identity (`PI_SESSION_ID`, `PI_SESSION_FILE`, `PI_PROVIDER`, `PI_MODEL`, `PI_REASONING_LEVEL`), controlled by `exposeSessionEnvironment`. Inherited values are deleted before the live ones are written, so a nested `pi` can never present its parent's identity as its own.
- Model pricing understands tiered rates (`cost.tiers`): the highest threshold the request's total input crosses prices the whole request. Existing single-threshold long-context pricing is unchanged. `samplingParams` and a configurable thinking-token-budget field are now honored by the OpenAI-compatible adapters and are settable from `models.json` and per-model overrides.
- The footer shows the latest assistant message's prompt-cache hit rate, and optional cache-miss notices explain a miss after a turn by comparing reported cache reads against the previous request, rather than guessing from elapsed time.
- `task_steps` no longer requires a step id to update the active step. The id is optional and defaults to the in-progress step, the per-turn context page now carries each step's id so any step can be targeted without a lookup, and completing the active step promotes the next pending one. Previously the only place ids appeared was the tool result, which scrolls out of context and is dropped by compaction, forcing an extra `list` call purely to relearn an id.

### Fixed

- A goal no longer completes while background work it owns is still running, and that work no longer wakes the agent once the goal has finished. Background tool tasks were stamped with a goal id only under a goal execution lease, which exists on continuation passes alone, so anything backgrounded during an ordinary foreground turn was recorded as unowned. The completion gate matches goal-owned tasks by that id and therefore could not see them, admitting `complete` while they ran; the terminal handoff treats a missing id as goal-independent and woke the parent unconditionally, producing further turns after the goal was already complete. Ownership is now read from the executing goal rather than from how the turn was admitted, so both gates see the same work. The lease id was always the active goal's, so attributed turns are unchanged, and terminals from work started with no goal active still wake the parent as before.

- `delegate` now rejects payload-bearing wrong-field input with a correction that names both the field that was sent and the one to use, instead of deleting the field and failing later under a different name. `cancel`, `interrupt`, `resume`, `retire` and `wait` sent `agentIds` previously produced `delegate <action> requires agentId`, which never mentioned `agentIds` and left the model retrying the same call unchanged; `follow_up` and `send` sent `task` produced the same dead end for `message`. Both now name the field and its singular or renamed counterpart, and say the call executed nothing; when the correct field was also sent, the correction adds which one to keep. Behavior change: `interrupt` with a `message` is now rejected. It previously dropped the message and reported the worker interrupted — a success path that silently discarded model-supplied text — so the correction states that the worker was not interrupted and the message was not delivered, and points at `follow_up` after `resume`. The plural spellings are never adopted as aliases, since a plural that works sometimes is the ambiguity these singular actions exist to avoid. Every other disallowed field keeps sanitising and proceeding, and `start` sent only `task` still adopts it as `instructions`.
- Path aliases no longer corrupt Windows paths that contain spaces or parentheses. A path segment excluded both characters, so extraction stopped at the first space and registered a prefix of the real path — `C:\Users\Dev\Documents\My Project\src\file.ts` minted an alias for `...\Documents\My`, a location that does not exist, and rewriting then substituted that alias back into the real mention, leaving `p/My Project\src\file.ts` in front of the model and sending every follow-up tool call to nothing. Segments now accept spaces and parentheses: a space continues a segment only when what precedes it is not already a complete filename, which stops one segment swallowing the gap between two paths or absorbing trailing prose; a relative path must still begin with a space-free segment, since it has no anchor of its own and would otherwise start at a preceding word; and a candidate containing a space must end in a file extension, because prose clears the length gate trivially. A trailing `)` is stripped only when it closes nothing inside the path, so `Program Files (x86)` and `Backup (old)` survive. Alias ids stay inside the token alphabet by folding unsupported characters, while the entry keeps the real spaced path that expansion returns.
- Path aliases are recognised regardless of separator or case on Windows. Extraction normalised a mention — folding backslashes and case-folding drive-letter paths — before minting an entry, while rewriting matched a fixed set of literal spellings exactly. Anything that normalised onto a known path without being byte-identical to a generated form minted an alias and a legend line that were then never used for it, so the transcript paid for both the alias and the full raw path on every turn it survived. Matching now accepts the same space extraction does, through a single filesystem-semantics abstraction chosen per path, so a drive-letter path is treated as case-insensitive even from a posix host while two case-differing posix paths remain distinct files that each keep their own alias.
- A permanent provider failure is no longer retried forever. The stream watchdog wraps any cause in one generic phrase, and the classifier matched that wrapper rather than the cause, so a hard configuration error was labelled a retryable stall and the retry ladder slept on a timer that, in a caller that never advanced it, never fired — the session blocked with no recovery path. Classification now runs against the wrapped cause; a bare wrapper with no cause attached is still a genuine stall, and the reported message keeps the full original text.
- Threshold compaction works for providers that report no usage. The between-turns check bailed out when no usage was present, leaving the whole cost of compaction to the next request, and the guard that suppresses repeated ineffective compactions compared against reported usage — permanently zero for such a provider — so a single ineffective compaction suppressed the trigger for the rest of the session. Both now fall back to the same live measurement the guard records, so the frontier has a real recovery path as the conversation grows.
- An unhandled promise rejection no longer ends the session. Node's default escalates one into an uncaught exception, which pi treats as fatal — correct for a synchronous throw, where process state may be unsound, but wrong for an isolated async failure. A single fire-and-forget promise left rejected by an extension (a status report whose transport is down, say) was enough to terminate a working session for a fault that is neither pi's nor fatal. Such a rejection is now reported in the transcript, with its stack, and the session continues. It is deliberately loud rather than swallowed: hiding these would mask real harness bugs, which is worse than the crash it replaces.
- The extension-only settled notification now waits for the idle lanes too. Goal auto-continue and the research lane are scheduled from the same prompt tail that decides whether a run has settled, on debounce timers, so a run whose next turn was already armed could be reported as settled and then contradicted milliseconds later. Both lane controllers now report whether a continuation is armed or running, and settlement requires that to be false — settled means nothing further happens without new input.
- `agent_end` is once again the last event on the session event channel. A settled-run notification was being emitted there as well as to extensions, so every completed turn trailed a second terminal-looking event. It is now delivered to extensions only.
- A custom message sent with `triggerTurn: false` can no longer land between a tool call and its result when the call has been backgrounded. The busy check consulted the foreground lease, which a backgrounded call outlives, and the public entry point acquires its own lease when nothing else is busy — so the check passed and the message was written straight into history. Such a message is now queued whenever a backgrounded call is still running, independent of lease ownership.
- A failed extension load no longer leaves its tools and commands registered. The load transaction restored event subscriptions, provider registrations and flags but not the tool or command registries, so a factory that registered either and then threw left them visible to the next turn.
- A UTF-8 byte-order mark no longer breaks settings, trust, profile, keybinding, theme, auth and skill files, or silently strips a skill's frontmatter. One helper now owns mark handling for both reading and the edit tools' round-trip.
- Fragmented Mistral tool-call streams are stitched back together. Accumulation keyed on the call id, which continuation chunks omit, so a synthesised id produced a second, empty call; keying on the chunk index recovers the original call, and an index of zero is no longer treated as absent.
- Resuming a session whose previous process was killed mid-write no longer corrupts it. The partial trailing line is isolated once the session header validates, instead of the next record being appended onto it; a file that fails header validation is never written to.
- A branch summary that hits its output cap is reported as a failure instead of being persisted as a complete summary.
- `Agent.reset()` during an active run throws instead of half-clearing state underneath the running loop.
...
- Two test-harness defects that only appeared on a clean checkout are fixed. The `pi-ai` source aliases used by the test projects were a hand-maintained list that had drifted from the package's export map — nine provider entry points were missing, so any project resolving through Vite rather than Node's `pi-source` condition fell through to `./dist`, which exists only on a machine that has built the package. The list is now derived from the export map itself, with a guard test that fails if it ever drifts again. Separately, the worker-context fork transaction test spawned its child without `--conditions=pi-source`, so the child resolved the same way and failed with ERR_MODULE_NOT_FOUND anywhere `dist` was absent.
...
- `pi update --models` has been removed. It printed a success message without refreshing anything, and no runtime mechanism exists to fetch the catalog.
- The unused `fullscreenCopyOnSelect` setting has been removed, superseded by tree-selection copy that is actually wired to the copy binding.

## [0.97.20] - 2026-08-29

### Changed

- The agent is now told to emit independent tool calls together in one message. The harness has executed a turn's tool calls concurrently for a long time, but nothing ever taught the model to send more than one per turn, so a turn needing six independent reads spent six round trips taking them one at a time. What existed was a hint — "batch independent reads" — surrounded by text pulling the other way: `edit` and `write` said "Call once", `tool_task` said "wait once, never poll", and the output contract is ULTRA-TERSE, all of which a model that already emits one call per turn reads as confirmation. `read`, `grep`, `find` and `ls` said nothing about batching at all, and now carry it in their own descriptions. `edit` and `write` move from "Call once" to "Call once per file", keeping the per-file contract they were actually protecting while making clear that different files may go out together. `ask-question` is marked emit-alone, since it executes sequentially and keeping it out of a batch avoids serialising the calls around it. Lean and minimal prompts carry a terser wording than the full class deliberately: both are enforced against a hard 8192-character ceiling that protects small-context models, and the full text would have pushed two fixtures over it.

### Fixed

- Path aliases no longer render as raw `p/...` tokens in streamed assistant text. Aliases exist to cut tokens on the wire and are meant to be expanded back wherever a person reads them, but the interactive UI handed the raw accumulated message straight to the renderer. Expansion had been wired only into the history-rebuild path — reached by reload, compaction and rewind, never by a turn completing normally — and the streaming component is created once at `message_start` and never re-rendered, so an alias the model wrote in its own prose stayed on screen for the life of that turn. Expansion is now recomputed from the raw message on every update, so an alias split across chunk boundaries renders literally for one update and heals on the next. It deliberately does not reuse the full display expander, which also walks tool-call arguments: on this path that runs per streamed chunk and would reintroduce the quadratic cost removed in 0.97.19. Arguments keep their existing expand-once-when-complete treatment.
- Thinking blocks are expanded for display, which the alias boundary never covered — its content walker matched only text and tool-call parts, so a thinking block passed through untouched. The expansion is opt-in rather than a widening of the existing text hook, because the same walker also builds the provider request, and a provider returning a thinking signature requires that text replayed byte for byte; rewriting it there would desync it from a signature computed over the original bytes. Only the display callers opt in.
- HTML session export expands path aliases, where it previously performed none at all and wrote exported transcripts containing raw alias tokens. The table is read through a loader that only issues selects: it does not go through the writer's open path, which would create a database and schema where none exists, and it skips the writer's checkpoint on close, so reading a session the exporter does not own performs no writes — asserted by hashing every file in the store directory across an export, which catches a stray WAL sidecar and not just a mutated database. Message entries, compaction summaries and branch summaries are all expanded. The store is resolved through the session's own agent directory; resolving it through the process-global one silently missed the table for any session built with a custom `agentDir`, degrading to the same raw output while the "table not found" warning made it look deliberate. The command-line export path, which opens a session file outside any session config, keeps the global default. A missing table degrades to unexpanded output with one warning naming the session; any other failure propagates.
...
- Path aliases no longer leak into anything the operator reads. Aliases (`p/grep.ts` for a full path) exist only to cut tokens on the wire; a model that reads one in its context writes it back in its prose and its tool arguments, and those reach the transcript verbatim, so the interactive UI was showing internal tokens instead of the operator's own paths. Expansion now happens at every boundary a person reads — interactive assistant text, tool-call arguments, tool output and bash commands, plus the final answer `pi -p` prints — the same automatic unwrapping `expandParams` already applies to tool arguments before a tool runs.
- Alias expansion no longer rebuilds its lookup table on every call, which made rendering cost O(messages x aliases) — quadratic in session length, and unnoticeable until a long session with a large table crawled. The map is memoized on the entries array like the rewriter's already was. Measured: expanding 2,000 messages against a 20,000-entry table went from 8,145ms to 2ms, and 5,000 messages against 5,000 entries from 3,608ms to 7ms. A scale gate now pins the curve shape (linear in messages, flat in table size) rather than a wall-clock number.
- An invented path alias now fails with a diagnostic that names the mistake. The model extrapolates ids from the legend's pattern (`p/module01.ts` is listed, so it reaches for `p/module02.ts`); an unminted token survived expansion untouched and failed as a literal relative path with `ENOENT ... /p/module02.ts`, an error describing a `p/` directory the project does not have, which cost a turn to misdiagnose. Such tokens are now rejected before the tool runs, naming the unminted id and pointing at the legend. A token that names a real file under an actual `p/` tree still passes through untouched, as do minted and reserved ids.
...
- Made `pi update --self`, `update pi`, and default `update` run the current standalone installer on Linux and Windows without npm.
- Made the TUI activity lane show live action labels and a blue status-only goal badge with transient completion, and consolidated TPS, token, model, and context metrics into pipe-wrapped footer rows.
