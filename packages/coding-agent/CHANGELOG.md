...
- Compiler and linter output through bash is reduced to grouped one-line diagnostics: rustc (through `cargo check`, `build`, `clippy`), `tsc` and `biome` reports render one line per diagnostic under its file (`line:col severity[code]: message`, identical diagnostics collapsed with a count, the lint name recovered from rustc's `#[warn(...)]` note), code frames survive only for errors, progress lines (`Compiling`, `Checking`, npm progress) and tsc's errors-per-file table are dropped, and every summary or unrecognized line is kept verbatim in place. Raw output is persisted and named in the notice as for search results. On the fixture corpus a `cargo check` with 30 warnings and one error shrinks by 73 %, a biome lint report by 65 %; the check gate holds both at 60 %.
