...
- Automatic commit records only successful typed edit/write paths, including worker writes on the same objective. A shell command that is not read-only git makes that commit fail `shell_mutation_unattributed`. An owned file whose bytes change without a new owned write fails `ownership_drift`. Root and lane bash refuse `git add -A`/`--all`/`.`/`-u` without paths, `git commit -a`/`--no-verify`, stash, `reset --hard`, `clean -f`, `checkout .`, `restore .`, and any `git push`.
- The git tree certified for a commit is frozen before JEV-024 and is the tree that is committed. A later edit fails the commit instead of approving a new tree. `completion_candidate` is read-only; a hook that asks to mutate does not pass. A push with no commit fails `stale_candidate` when HEAD is no longer the semantic candidate.
