...
- Foreground idle wait yields while an evaluation or armed continuation is still occupancy, instead of retrying on microtasks and starving that work. A running worker or a detached background tool does not block the next foreground turn, so a worker terminal and one background call can be delivered while other work is still in flight. The goal loop reports `worker_in_flight` instead of waiting for that worker. An edit that ran and failed still names its path on the worker claim. An operator verification dismissal is written while a continuation is only armed. Lane bash does not treat `git` inside `echo` or `printf` as a git invocation. A Decision graph title that cannot fit `Follow current` keeps Hide and the view chips.
