...
- BLOCKER: the tool gate's `replan` verdict cancelled the whole foreground turn (System One's cancel lever), which killed every sibling tool call and the operator's own request and left an empty aborted assistant message; typing "continue" repeated it. The verdict now refuses that one call only and the turn goes on. The verdict itself was wrong on plain sessions: relevance was judged against an empty objective, so every call read as "not relevant"; relevance is now evaluated only against a real plan step or goal. Inside the objective loop a System One cancel of the root turn is a re-route, not a stop.
