...
- Windows suite and agent-dir cleanup removes a tree with the same transient EPERM retry as an atomic rename, and the suite temp directory is the real path of the temp root. A short TEMP alias was leaving `rm` denied after the worker test had passed.
